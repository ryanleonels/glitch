...
function calcDynamic() {
  return (
    ...
    (game.upgrades.includes(13) && (game.challenge % 2 === 1 || game.chal9 === 1) ? 2 : 1)
  );
}

function getChalFact() {
  if (!(game.challenge === 6 || game.challenge === 7) && (game.boostUnlock==1||game.factorShifts==7)) {
    return 4
...
function getFactorBulk() {
  if (game.OP >= V(game.factorBoosts + 3, 1) || (game.factorBoosts>=25 && !game.upgrades.includes(16))) {
    let i = 1;
    while (
    ...
    return (game.bulkBoost === 1 && game.upgrades.includes(8)) ? i : 1;
  }
  return 0;
}
...
function getSumOfChallenges() {
  ...
  return r + game.chal8Comp + game.chal9Comp;
}
...
function getIncrementyRate(x) {
  ...
  return EN(
    ((ordRate ** (game.factorBoosts >= 24 ? 0.9 : 0.85) * x) / 1000) *
      2 ** game.iups[1] *
...
function calcTotalMultWOFactor() {
  return (
    ...
    (game.chal8 === 0 ? game.assCard[0].mult.toNumber() : 1) *
    1.2 ** game.dups[2] *
    (game.aups.includes(4) ? Math.log10(Math.log10(1e10 + game.OP)) : 1) *
    getChalFact() / (game.factorShifts == 7 ? 2 : 1)
  );
}
...
function getBoostFromBoosters(check = 0) {
  return game.upgrades.includes(6) || check === 1
    ? Math.max(
        game.leastBoost <= 11 ? Math.min(25,Math.max(5, game.factorBoosts)) + 9 * Math.max(game.boosters, 0) ** 0.4 : 0,
        Math.min(25,Math.max(5, game.factorBoosts)) + Math.max(game.boosters, 0) ** 0.6
      )
    : 1;
}
...
function getFSCost(fs = game.factorShifts) {
  return Math.round((factorShiftCosts[fs]) * 
    (((game.challenge === 3 && fs == 6) || (game.chal8 === 1 && (fs == 4 || fs == 5))) ? 0.1 : 1)) * 
    (game.chal8 === 1 && fs == 2 ? 0.1 : 1) * 
    (game.chal8 === 1 && fs == 3 ? 4e-6 : 1) *
    ((game.challenge === 1 || game.challenge === 7) && (fs == 0 || fs == 1) ? 10 : 1)
}
...
function calcOPPS(fs = game.factorShifts) {
  return (
    ...
    ((game.upgrades.includes(7) ? 10 : 0) + (game.leastBoost <= 14 ? 15 : 0)) *
...
function calcIncrementyMult(i = game.incrementy) {
  return ExpantaNum(i)
    .add(10)
    .log10()
    .pow(ExpantaNum(1.05).pow(game.iups[0]+1))
...
function getChal8Goal(x) {
  return x >= 3 && game.leastBoost >= 1.5
    ? Infinity
    : ((x < 3 ? challengeGoals[7][x] : 2 ** ((x * (x + 1)) / 2) * 3e10) / 
    (game.sfBought.includes(63)?game.assCard[0].mult.toNumber():1));
}
...
function calcBase(n = game.factorShifts) {
  let b =
    10 -
    (game.challenge === 4 || game.challenge === 7 ? 0 : n) +
...
function formateNum(n, p, d) { // n is number, p is precision and d is decimal points below 1k
  let array = ["", " K", " M", " B", " T", " Qa", " Qt", " Sx", " Sp", " Oc", " No", " Dc", " UDc", " DDc", " TDc", " QaDc", " QtDc", " SxDc", " SpDc", " ODc", " NDc", " Vg", " UVg", " DVg", " TVg", " QaVg", " QtVg", " SxVg", " SpVg", " OVg", " NVg", " Tg", " UTg", " DTg", " TTg", " QaTg", " QtTg", " SxTg", " SpTg", " OTg", " NTg", " Qd", " UQd", " DQd", " TQd", " QaQd", " QtQd", " SxQd", " SpQd", " OQd", " NQd", " Qi", " UQi", " DQi", " TQi", " QaQi", " QtQi", " SxQi", " SpQi", " OQi", " NQi", " Se", " USe", " DSe", " TSe", " QaSe", " QtSe", " SxSe", " SpSe", " OSe", " NSe", " St", " USt", " DSt", " TSt", " QaSt", " QtSt", " SxSt", " SpSt", " OSt", " NSt", " Og", " UOg", " DOg", " TOg", " QaOg", " QtOg", " SxOg", " SpOg", " OOg", " NOg", " Nn", " UNn", " DNn", " TNn", " QaNn", " QtNn", " SxNn", " SpNn", " ONn", " NNn", " Ce", " UCe"]
  if (d == undefined || !isFinite(d) || d < 0) d = 0;
  if (p == undefined || !isFinite(p) || p < 0) p = 3 + game.prec;
  if (!isFinite(n)) return n;
  if (Math.abs(n) < 1000) return (n).toFixed(d);
  if (Math.abs(n) < 10 ** (6 + game.start * 3)) return commafy(n);
  if (game.number == 1) {
    let exponent = Math.floor(Math.log10(Math.abs(n))/3)
    let mantissa = n / (1000 ** exponent)
    if (mantissa >= 1000 - 10 ** (-1 * p + 3) / 2) mantissa = 1000 - 10 ** (-1 * p + 3)
  return mantissa.toPrecision(p) + array[exponent]
  } else {
    let exponent = Math.floor(Math.log10(Math.abs(n)))
    let mantissa = n / (10 ** exponent)
    if (mantissa >= 10 - 10 ** (-1 * p + 1) / 2) mantissa = 10 - 10 ** (-1 * p + 1)
    return mantissa.toPrecision(p) + "e" + exponent
  }
}

function formatePow(n, p, d) { // same as above, but with 10^n
  let arrayBelowE33 = ["", " K", " M", " B", " T", " Qa", " Qt", " Sx", " Sp", " Oc", " No"]
  let arrayOnes = ["", "U", "D", "T", "Qa", "Qt", "Sx", "Sp", "O", "N"]
  let arrayTens = ["", "Dc", "Vg", "Tg", "Qd", "Qi", "Se", "St", "Og", "Nn"]
  let arrayHundreds = ["", "Ce", "Dn", "Tc", "Qe", "Qu", "Sc", "Si", "Oe", "Ne"]
  if (d == undefined || !isFinite(d) || d < 0) d = 0;
  if (p == undefined || !isFinite(p) || p < 0) p = 3 + game.prec;
  if (n < 3) return (10 ** n).toFixed(d);
  if (!isFinite(n)) return n;
  let exponent = Math.floor(n)
  let mantissa = 10 ** (n - Math.floor(n))
  if (exponent >= 10 ** (6 + game.start * 3)) {
    return "e" + formateNum(n, p, d)
  } else if (exponent > 3003 || (game.number == 0 && exponent >= 6 + game.start * 3)) {
    if (mantissa >= 10 - 10 ** (-1 * p + 1) / 2) mantissa = (10 - 10 ** (-1 * p + 1))
    return mantissa.toPrecision(p) + "e" + commafy(exponent)
  } else if (game.number == 1 && exponent >= 6 + game.start * 3){
    let exponentDiv3 = Math.floor(exponent/3)
    let mantissaMultTenPow = mantissa * 10 ** (exponent % 3)
    if (mantissaMultTenPow >= 1000 - 10 ** (-1 * game.prec + 1) / 2) mantissaMultTenPow = (1000 - 10 ** (-1 * p + 3))
    if (exponentDiv3 < 10.5) return mantissaMultTenPow.toPrecision(p) + arrayBelowE33[exponentDiv3]
    return mantissaMultTenPow.toPrecision(p) + " " + arrayOnes[(exponentDiv3 - 1)%10] + arrayTens[(Math.floor((exponentDiv3 - 1)/10))%10] + arrayHundreds[Math.floor((exponentDiv3 - 1)/100)]
  } else return commafy(10 ** n)
}

function formateOrd(n, p, d) { // same as above, but after 1e265 use displayOrd
  let array = ["", " K", " M", " B", " T", " Qa", " Qt", " Sx", " Sp", " Oc", " No", " Dc", " UDc", " DDc", " TDc", " QaDc", " QtDc", " SxDc", " SpDc", " ODc", " NDc", " Vg", " UVg", " DVg", " TVg", " QaVg", " QtVg", " SxVg", " SpVg", " OVg", " NVg", " Tg", " UTg", " DTg", " TTg", " QaTg", " QtTg", " SxTg", " SpTg", " OTg", " NTg", " Qd", " UQd", " DQd", " TQd", " QaQd", " QtQd", " SxQd", " SpQd", " OQd", " NQd", " Qi", " UQi", " DQi", " TQi", " QaQi", " QtQi", " SxQi", " SpQi", " OQi", " NQi", " Se", " USe", " DSe", " TSe", " QaSe", " QtSe", " SxSe", " SpSe", " OSe", " NSe", " St", " USt", " DSt", " TSt", " QaSt", " QtSt", " SxSt", " SpSt", " OSt", " NSt", " Og", " UOg", " DOg", " TOg", " QaOg", " QtOg", " SxOg", " SpOg", " OOg", " NOg", " Nn", " UNn", " DNn", " TNn", " QaNn", " QtNn", " SxNn", " SpNn", " ONn", " NNn", " Ce", " UCe"]
  if (d == undefined || !isFinite(d) || d < 0) d = 0;
  if (p == undefined || !isFinite(p) || p < 0) p = 3 + game.prec;
  if (!isFinite(n)) return n;
  ...
  if (Math.abs(n) < 10 ** (6 + game.start * 3)) return commafy(n);
  if (n >= 1e265) {
    return "g<sub>" + displayOrd(n - 9.9e269, 3) + "</sub> (10)";
