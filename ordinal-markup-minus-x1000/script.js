...
load()
function load() {
  let loadgame = JSON.parse(localStorage.getItem("ordinalMarkupSaveMinusX1000"))
  if (loadgame != null) {
    loadGame(loadgame)
  }
}
...
function save() {
  localStorage.setItem("ordinalMarkupSaveMinusX1000", JSON.stringify(game))
}
...
function maxall() {
  ...
  if (game.challenge==1) {
    ...
    if (game.OP<10**265) {
      ...
      if (bulk>0) game.OP -= ((2**bulk)-1)*(100*2**game.succAuto)
      game.succAuto += bulk
      bulk=Math.floor(Math.log(1+game.OP/(100*2**game.limAuto))/Math.log(2))
      if (bulk>0) game.OP -= ((2**bulk)-1)*(100*2**game.limAuto)
      game.limAuto += bulk
    } else {
      game.succAuto=game.OP
      game.limAuto=game.OP
