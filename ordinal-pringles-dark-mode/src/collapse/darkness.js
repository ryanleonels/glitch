...
function updateDarknessControlHTML(mode){
    switch (mode) {
        ...
        case 3:
            ...
            break;
        case 5:
            DOM('dupC5').innerText = `${boolToReadable(!data.darkness.enableChalInc, 'EDT')} Challenge and Incrementy inside Darkness`
            break;
        default:
            break;
    }
...
function darknessControl(mode){
    ...
    if(mode===5){
        if (data.darkness.darkened && data.darkness.enableChalInc) { // enable to disable
            if (data.chal.html != -1) {
                chalExit()
            }
        }
        data.darkness.enableChalInc = !data.darkness.enableChalInc
        if (data.darkness.darkened) {
            chalEnter(7, true)
            if (data.darkness.enableChalInc) data.chal.html = -1 // disable to enable
        }
    }
...
function darken(force = false){
    ...
    data.darkness.darkened && !force ? chalExit(true, true) : chalEnter(7, true)

    DOM('darken').innerText = data.darkness.darkened ? 'Enter the Darkness' : 'Escape'
    data.darkness.darkened = !data.darkness.darkened

    if (data.darkness.darkened && data.darkness.enableChalInc) data.chal.html = -1
}
