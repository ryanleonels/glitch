...
let base2Req = () => data.collapse.times > 0 ? 1 : 1 //33
function updateMarkupHTML(){
    ...
    DOM("factorShiftButton").innerHTML = data.ord.base===3?(data.boost.times>0||data.collapse.hasSluggish[0])&&false?`Perform a Factor Shift<br>Requires: ?????`:`Perform a Factor Shift<br>Requires: Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))`:
...
function calcOrdPoints(ord = data.ord.ordinal, base = data.ord.base, over = data.ord.over, trim=0) {
    if ((base < 1) || (base === 1 && ord >= 1) || (base === 2 && ord >= 65536) || (ord === Infinity)) return Decimal.tetrate(10, Number.MAX_VALUE);
...
function factorShift(isAuto = false){
    ...
    if(data.markup.shifts === 7 && !isAuto){
        if(data.ord.isPsi && data.ord.ordinal >= GRAHAMS_VALUE && (data.boost.times == 0 || data.boost.times >= base2Req())) {
            if (data.boost.times == 0) return boost(true)
        }
        else {
            return //createAlert("Failure", "Insufficient Ordinal", "Dang.")
        }
    }
