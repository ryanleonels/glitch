...
function factorEffect(n){
    ...
    return ((data.factors[n]+(1+add))*mult*bup7Effect())*(Math.max(1+(Math.min(data.markup.shifts,7)-n-1)/10, 1)**[1, 1, 1, 1, 1.3, 1.9, 2.2, 2.3][Math.min(data.markup.shifts,7)])
}
...
function buyMaxFactor(){
    ...
    for (let i = Math.min(data.markup.shifts,7)-1; i >= 0; i--){
        if(!hasFactor(i)) break
        while (data.markup.powers >= Math.pow(10 ** (i + 1), Math.pow(2, data.factors[i]))) buyFactor(i);
    }
}
