...
function chalEnter(i, force=true){
    if((data.chal.completions[i] === 3 && !force) || data.chal.html != -1) return
...
function chalExit(darkness = false, darkens = false){
    if(data.darkness.darkened && data.chal.active[7] && !darkness && (!data.darkness.enableChalInc || data.chal.html === -1)) darken(true)
    for (let i = 0; i < data.chal.active.length; i++) {
        data.chal.active[i] = false
        updateChalHTML(i)
    }
    if (data.darkness.darkened && !darkens) {
        data.chal.html = -1
        chalEnter(7, true)
    }
    data.chal.html = -1
...
function chalComplete(){
    if(data.chal.html === -1 || (data.darkness.darkened && !data.darkness.enableChalInc)) return
    if(data.chal.completions[data.chal.html]==3) return
    ...
    if(currency>=chalGoals[data.chal.html][data.chal.completions[data.chal.html]] && ex){
        ...
        chalExit()
        //if (data.darkness.darkened) chalEnter(7, true)
        //data.chal.html = -1
    }
}
