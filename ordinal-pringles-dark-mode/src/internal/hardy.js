...
{
  let ord1 = ExpantaNum(ord.toString());
...
function psiHardy(ord, base) {
  ...
  if (base === 2) { // psi base 2 - in reverse order, individual values are only available up to omega fixed point (per-level value above that point) - bottom values are 3 instead of 2 as arrays with 2,2 as starting values are degenerate
    if (ord.gte(2**297)) return "s(2,3{1,,2,,2}2)"; // Ω_(I+1) level (using Strong Array Notation) (highest level reachable below Number.MAX_VALUE)
    if (ord.gte(17)) return "s(2,3{1,,1,,2}2)"; // I level (using Strong Array Notation)
    if (ord.gte(16)) return "{2,3[1[2/<sub>1[1[2/<sub>1,2</sub>2]2]2</sub>2]2]2}"; // θ(Ω_Ω) = omega fixed point (limit of Bird's Array Notation)
    if (ord.gte(15)) return "{2,3[1[2/<sub>1[1[1/2~2]2]2</sub>2]2]2}"; // θ(Ω_Γ0)
