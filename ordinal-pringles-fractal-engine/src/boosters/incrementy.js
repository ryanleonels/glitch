function updateIncrementyHTML(){
    ...
    DOM(`iup12`).innerText = `[RUP7] ${iupDesc[12]} (${formatWhole(data.incrementy.rebuyableAmt[6])})\n${format(getRebuyableCost(6))} Incrementy\nCurrently: ${format(iupEffects[12]())}x`
...
const iupDesc = ['Double Incrementy Gain', 'Triple Dynamic Gain', 'Dynamic Factor boosts Incrementy gain',
    ...
    'Double Negative Charge gain and Cap', 'Gain a level of the second Darkness Upgrade', 'Boost Cardinal gain', 'Fractal Engine Level 0: Incrementy x10 every tick'
]
const iupCosts = [1, 1, 1, 2e6, 2e5, 1e10, 3e4, 1e8, 1e12, 1e100, 1e150, 1e200, new Decimal("1e750")]
let rebuyableCostBases = [20, 1000, 100, 1e150, 1e150, 1e150, new Decimal("1e750")]
let rebuyableCostScalings = [2, 2, 2, 30, 40, 10, 10]
...
function initIUPs(){
    let rows = [DOM('iupRow0'), DOM('iupRow1'), DOM('iupRow2'), DOM('iupRow3'), DOM('iupRow4')]
    let total = 0
    for (let i = 0; i < rows.length; i++) {
        let r = i === 0 || i === 5
        for (let n = 0; n < 3; n++) {
            if (total < iupCosts.length) {
                ...
                if (total == iupCosts.length - 1) iup.style = "width: 46rem;"
                rows[i].append(iup)
                ++total
            }
        }
...
let iup13Effect = () => data.incrementy.rebuyableAmt[6]

let iupEffects = [iup1Effect, iup2Effect, iup3Effect, iup4Effect, iup5Effect, iup6Effect, iup7Effect, iup8Effect, iup9Effect, iup10Effect, iup11Effect, iup12Effect, iup13Effect]
