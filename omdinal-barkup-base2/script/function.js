...
function calcCardExponent(s) {
  return (2/3) + (5 - Math.min(Math.log10(Math.max(1000, s)), 5)) ** (5/3);
}
...
function changeBase2() {
  game.enableBase2 = 1 - game.enableBase2;
  if (game.enableBase2 == 0 && game.factorShifts == 8) {
    game.factorShifts = 7;
    game.factors.length = 7;
    game.base = calcBase();
  }
  render();
  save();
...
function calcFactorBoostTime() {
  ...
  if (game.enableBase2) return (
    Math.max(
      1 / game.boostAuto.toNumber(),
      fbt + EN(117 / speed).toNumber()
    ) / getFBmult()
  );
  else return (
    Math.max(
      1 / game.boostAuto.toNumber(),
      fbt + EN(BHO / 1e270 / speed).times(EN(3).pow(getSingLevel() - 1)).toNumber()
    ) / getFBmult()
  );
}
