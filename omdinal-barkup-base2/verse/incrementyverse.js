...
const orb4UnlockReq = 10000
const orb5UnlockReq = Infinity
...
function incrementyverseLoop(ms,off=0) {
  game.base = (game.enableBase2 ? 2 : 3)
  ...
  if (game.orbUnlock == 1) {
    if ((!game.orbEffectExp) || (EN(game.orbEffectExp).isNaN())) game.orbEffectExp = EN(0)
    game.orbEffectExp = EN(game.orbEffectExp).add(EN(ms).div(1000).times(calcOrbSpeedMult()))
  }
...
function incrementyverseRender() {
  ...
  let hardyDisplay = "H<sub>" + OPord + "</sub><invisible>l</invisible>(" + (3 - game.enableBase2) + ")=" + hugeHardy(game.bigBrainOrd, game.enableBase2 ? 2 : 3);
  if (game.colors == 1) {
...
They are based on your Singularity level.`
  ...
  get("energyOrbText2").innerHTML=`Efficiency: +${beautifyEN(EN(1).sub(getOrbEffectMult()).times(100).times(calcOrbSpeedMult()))}%/sec (${beautifyEN(calcOrbSpeedMult())}x speed)`
  ...
  if (getfractalshift() >= orb5UnlockReq) {
    get("energyOrb5Desc").innerHTML = '?????<br>Boosted: '
  ...
  updateOrb()
  get("fractalShifts").innerHTML = "You have " + (game.fractalUpgrades[4] >= 1 ? " ( " : "") + `${game.fractalShift}` + (game.fractalUpgrades[4] >= 1 ? (" + " + game.fractalUpgrades[4] + " ) ") : "") + " Fractal Shifts"
...
function displayHugeOrd(x) {
  ...
  return "ψ(" + (game.base == 2 ? realDisplayHugeOrd2(x.div(4).floor()) : realDisplayHugeOrd(x.div(4).floor())) + ")";
}
...
function realDisplayHugeOrd(x,layer=0) {
  ...
  if (x.layer > 0) {
    ...
    if (layer1 >= 3) {
      return `Ω<sub>${realDisplayHugeOrd(EN(layer1),nly)}</sub>`
    }
    k.layer--
    if (game.buchholz == 0) return `φ<sub>${realDisplayHugeOrd(k,nly)}</sub>(Ω+1)`
    return (game.buchholz==2) ? `Ω<sub>2</sub>^(${realDisplayHugeOrd(k,nly)})`:`Ω<sub>2</sub><sup>${realDisplayHugeOrd(k,nly)}</sup>`
  }
  if (x.gte(EN(3).pent(5))) {
    ...
    if (arrowLevel === 2) {
      ...
      return (game.buchholz == 0) ? `ζ<sub>${realDisplayHugeOrd(zOrd,nly)}</sub>` : `${game.buchholz==2?"Ω<sub>2</sub>^(2)×":"Ω<sub>2</sub><sup>2</sup>"}${realDisplayHugeOrd(EN(3).pow(EN(arrowHeight).logBase(3).floor()),nly)}`
    }
    if (arrowLevel === 3) {
      return (game.buchholz == 0) ? "Γ<sub>Ω+1</sub>" : `${game.buchholz==2?"Ω<sub>2</sub>^(Ω)":"Ω<sub>2</sub><sup>Ω</sup>"}`
    }
    if (game.buchholz == 0) return `φ<sub>${realDisplayHugeOrd(EN(arrowLevel),nly)}</sub>(Ω+1)`
    return (game.buchholz==2) ? `Ω<sub>2</sub>^(${realDisplayHugeOrd(EN(arrowLevel),nly)})`:`Ω<sub>2</sub><sup>${realDisplayHugeOrd(EN(arrowLevel),nly)}</sup>`
    //ζ
    //3^^^5 (height 2) -> 6
    //3^^^8 (height 5) -> 9
    //3^^^11 (height 8) -> 12
  ...
  if (x.gte(EN(3**27).tetr(3))) {
    ...
    if (ordSub.gte(3**27)) {
      ...
      if (ordSub2.gte(3**27)) {
        ...
        return (game.buchholz == 0) ? `ε<sub>ε<sub>ε<sub>${realDisplayHugeOrd(ordSub3,nly+2)}</sub></sub></sub>` : `Ω<sub>2</sub>${game.buchholz==2?"×":""}Ψ<sub>1</sub>(Ω<sub>2</sub>${game.buchholz==2?"×":""}Ψ<sub>1</sub>(Ω<sub>2</sub>${realDisplayHugeOrd(EN(3).pow(pt3.logBase(3).floor()),nly+2)}))`
      }
      return (game.buchholz == 0) ? `ε<sub>ε<sub>${realDisplayHugeOrd(ordSub2,nly+1)}</sub></sub>` : `Ω<sub>2</sub>${game.buchholz==2?"×":""}Ψ<sub>1</sub>(Ω<sub>2</sub>${realDisplayHugeOrd(EN(3).pow(pt2.logBase(3).floor()),nly+1)})`
    }
    let ordE = (game.buchholz == 0) ? `ε<sub>${realDisplayHugeOrd(ordSub,nly)}</sub>` : `Ω<sub>2</sub>${realDisplayHugeOrd(EN(3).pow(ordSub.logBase(3).floor()),nly)}`
...
function realDisplayHugeOrd2(x,layer=0) {
  let ords = [
    "1", "ω", "Ω", "Ωω",
    "Ω<sup>ω</sup>", "Ω<sup>ψ(Ω)</sup>", "Ω<sup>ψ(Ωω)</sup>", "Ω<sup>ψ(Ω<sup>ω</sup>)</sup>",
    "Ω<sub>ω</sub>", "Ω<sub>ψ(Ω)</sub>", "Ω<sub>ψ(Ωω)</sub>", "Ω<sub>ψ(Ω<sup>ω</sup>)</sub>",
    "Ω<sub>ψ(Ω<sup>ψ(Ω)</sup>)</sub>", "Ω<sub>ψ(Ω<sup>ψ(Ωω)</sup>)</sub>", "Ω<sub>ψ(Ω<sup>ψ(Ω<sup>ω</sup>)</sup>)</sub>", "Ω<sub>ψ(Ω<sub>ω</sub>)</sub>",
    "Ω<sub>Ω</sub>"];
  // ordinals intentionally spread out to make the higher ones visible across the range of pre-fractal engine era (instead of the expected instant Ω_Ω)
  if (x.lt(1)) return x.toString() // 0 base 2 @ 0 base 3
  if (x.lt(2)) return ords[0] // 1 base 2 @ 1 base 3
  if (x.lt(3)) return ords[1] // ω base 2 @ 2 base 3
  if (x.lt(3**2)) return ords[2] // Ω base 2 @ Ω base 3
  if (x.lt(3**3)) return ords[3] // Ωω base 2 @ Ω^2 base 3
  if (x.lt(3**6)) return ords[4] // Ω^ω base 2 @ Ω^Ω base 3
  if (x.lt(3**9)) return ords[5] // Ω^ψ(Ω) base 2 @ Ω^(Ω2) base 3
  if (x.lt(3**18)) return ords[6] // Ω^ψ(Ωω) base 2 @ Ω^(Ω^2) base 3
  if (x.lt(3**27)) return ords[7] // Ω^ψ(Ω^ω) base 2 @ Ω^((Ω^2)2) base 3
  if (x.lt(EN(3).pow(3**6))) return ords[8] // Ω_ω base 2 @ ε(Ω2) base 3
  if (x.lt(EN(3).pow(3**9))) return ords[9] // Ω_ψ(Ω) base 2 @ ε(Ω2)^(Ω^Ω) base 3
  if (x.lt(EN(3).pow(3**18))) return ords[10] // Ω_ψ(Ωω) base 2 @ ε(Ω2)^(Ω^(Ω^2)) base 3
  if (x.lt(EN(3).pow(3**27))) return ords[11] // Ω_ψ(Ω^ω) base 2 @ ε(Ω2)^(Ω^((Ω^2)2)) base 3
  if (x.lt(EN(3).pow(EN(3).pow(3**6)))) return ords[12] // Ω_ψ(Ω^ψ(Ω)) base 2 @ ε(Ω2)^ε(Ω2) base 3
  if (x.lt(EN(3).pow(EN(3).pow(3**9)))) return ords[13] // Ω_ψ(Ω^ψ(Ωω)) base 2 @ ε(Ω2)^ε(Ω2)^(Ω^Ω) base 3
  if (x.lt(EN(3).pow(EN(3).pow(3**18)))) return ords[14] // Ω_ψ(Ω^ψ(Ω^ω)) base 2 @ ε(Ω2)^ε(Ω2)^(Ω^(Ω^2)) base 3
  if (x.lt(EN(3).pow(EN(3).pow(3**27)))) return ords[15] // Ω_ψ(Ω_ω) base 2 @ ε(Ω2)^ε(Ω2)^(Ω^((Ω^2)2)) base 3
  return ords[16] // Ω_Ω base 2 @ ε(Ω^2) base 3
}
...
function updateFractalEngine(ms) {
  ...
  for (let ind=game.fractalEngine.length-1;ind>=0;ind--) {
    ...
    if (mach.charge.isNaN()) mach.charge=EN(0)
  ...
  for (let i in game.fractalEngine) {
    ...
    Buy Fractal Engine Level ${beautifyEN(mach.level)}<br>Owned: ${beautifyEN(mach.own)}<br>Cost: ${beautifyEN(FGH(mach.level.add(1),EN(getFractalBase())).times(EN(game.fractalShift).tetr(2)))} Incrementy`
    let tempown = mach.own
...
${(game.fractalEngine.some(mach => {return mach.own.gte(1e140)})?`<br><b style="color:red;">Your Fractal Engine gets slower after 1e140 copies (x --> 1e140(x/1e140)^(0.9999))</b>`:"")}`
  ...
  <br>${(getOrbs(getfractalshift()+1)-getOrbs(getfractalshift())==0?`Next Orb in ${Math.floor((getOrbs()/2+1)**(1/0.666))-getfractalshift()} FS`:`+1 Energy Orb`)}
...
function buyFractalEngine(x) {
  let cost=FGH(game.fractalEngine[x-1].level.add(1),EN(game.fractalBase)).times(EN(getfractalshift()).tetr(2))
