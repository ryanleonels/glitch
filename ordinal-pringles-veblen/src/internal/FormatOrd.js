...
function displayOrd(ord,over,base,trim = data.ord.trim) {
    if(data.ord.veblen) return displayVeblenOrd(ord,over,base,trim)
...
function displayInfiniteOrd(ord, over, base, trim = data.ord.trim){
    if(data.ord.veblen) return displayInfiniteVeblenOrd(ord,over,base,trim)
...
function displayHierarchyOrd(ord,over,base,trim = data.ord.trim) {
    if(data.ord.veblen) return displayHierarchyVeblenOrd(ord,over,base,trim)
    ord = Decimal.floor(ord)
    over = Decimal.floor(over)
    if(trim <= 0) return `...`
...
function displayHierarchyVeblenOrd(ord,over,base,trim = data.ord.trim) {
    ord = Decimal.floor(ord)
    over = Decimal.floor(over)
    if(trim <= 0) return `...`
    if(ord < base) return ord.plus(over)
    ...
    const amount = Decimal.floor(ord.div(magnitudeAmount))
    let finalOutput = "&phi;(1)"
    if (magnitude.gt(1)) finalOutput = "&phi;("+displayHierarchyVeblenOrd(magnitude, 0, base)+")"
    if (amount.gt(1)) finalOutput += amount
    const firstAmount = amount.times(magnitudeAmount)
    if(ord.sub(firstAmount).gt(0)) finalOutput += "+" + displayHierarchyOrd(ord.sub(firstAmount), over, base, trim - 1)
    if(data.gword) finalOutput = finalOutput.replaceAll("&phi;","<img src='https://cdn.discordapp.com/emojis/916425545770745856.webp?size=24'>")
...
function displayPsiOrd(ord, trim = data.ord.trim, base = data.ord.base) {
    if(data.ord.veblen && D(ord).lte(BHO_VALUE)) return displayPsiVeblenOrd(ord,trim,base)
...
function displayInfinitePsiOrd(ord, trim = data.ord.trim, base = data.ord.base) {
  if(data.ord.veblen && D(ord).lte(BHO_VALUE)) return displayInfinitePsiVeblenOrd(ord,trim,base)  
...
const ordMarksVeblen = [
    ...
    "&phi;(y,0,0,0)",
    "&phi;(1@y)",
    "&phi;(1{1:0}0)", //"&psi;(&phi;(1,Ω+1))", //"&psi;(ε<sub>Ω+1</sub>)"
]
...
function displayInfinitePsiVeblenOrd(ord, trim = data.ord.trim, base = data.ord.base) {
    if (D(ord).gt(BHO_VALUE)) return displayInfinitePsiOrd(ord, trim, base) // not yet supported
    return displayPsiVeblenOrd(Math.floor(ord), trim, base) // "infinite" isn't in the supported range yet (as BHO_VALUE is less than Number.MAX_VALUE)
}
