...
function loop(unadjusted, off = 0) {
  ...
  if ((game.challenge>0||game.chal9==1||game.chal0==1) && !(game.chal8===1) && game.leastBoost <= 5.5 && game.qolSM.acc === 1) {
    if ( // !(1) = false iirc
      game.OP >= challengeGoals[game.chal0==1?9:(game.chal9==1?8:game.challenge - 1)][Number(game.qolSM.nc8) - 1]
    ) {
      completeChallenge();
    }
  }
  ...
  if (inChal(8)) game.decrementy = Math.min(game.decrementy + getDecrementyRate(ms), Number.MAX_VALUE); //oof, sniped I WAS SUPPOSED TO DO c8 re
  ...
  if (ms > 0) {
    if ((game.upgrades.includes(2) || game.leastBoost <= 5.5) && game.autoOn.max === 1) {
      ...
      if (game.bAutoLoop.max >= 1000 / buptotalMute) {
        ...
        if ((game.OP < (inChal(5) && game.factorShifts >= 2 || game.factorShifts==(7+game.chal0) ? Infinity : getFSCost(game.factorShifts)) && ((game.challengeCompletion[game.challenge-1]==3)||(!(game.challenge > 0) && !(game.chal8)&&game.chal9==0&&game.chal0==0) || game.OP < (challengeGoals[game.challenge - 1] == undefined ? 1e300 : challengeGoals[game.challenge-1][game.challengeCompletion[game.challenge - 1]]))) || game.OP >= 1e265) {
          maxInfStuff();
        }
      }
    }
...
function render() {
  ...
  if (game.boostUnlock === 1) {
    ...
    if (!(game.challenge>0||game.chal8==1||game.chal9==1||game.chal0==1)) {
      ...
      get("finishChallenge").innerHTML =
        `Complete the challenge!<br>${beautify(
          game.chal8 === 1
            ...
            : challengeGoals[game.chal0==1?9:(game.chal9==1?8:game.challenge - 1)][
                game.chal0==1?game.chal0Comp:(game.chal9==1?game.chal9Comp:game.challengeCompletion[game.challenge - 1])
              ]
        )} OP`;
    }
  } else {
  ...
  get("ordinalPointsDisplay").innerHTML =
    `You have ${fbvps>=10&&game.OP>=1e270?calcOPPS()*game.msint/1000:beautify2(game.OP)} Ordinal Points`;
  ...
  get("factorShift").innerHTML =
    ...
    (inChal(5) && game.factorShifts >= 2
      ? "Infinity"
      : game.factorShifts >= 7
      ? (game.factorShifts >= 8 + game.chal0 - game.boostUnlock)
        ? "Infinity"
  ...
  get("runChal").textContent =
    game.chal8 == 1 || game.chal9 == 1 || game.chal0 == 1
      ? "You're currently running Challenge " + (game.chal8 == 1?8:(game.chal9 == 1?9:0))
      : (!inAnyChal())
  ...
  get("incrementyText2").textContent =
    "You are getting " +
    beautifyEN(getIncrementyRate(1000).div((game.chal9) ? 1e15 : 1)) +
    " incrementy per second";
  get("iup1").innerHTML =
    "Base Incrementy multiplier is raised to the 1.05<br>Cost: " +
    beautify(EN(game.iups[0]).gte(120)?Infinity:EN(100000).pow(game.iups[0]).times(100000));
  get("iup2").innerHTML =
    "Double the production of incrementy<br>Cost: " +
    beautify(EN(game.iups[1]).gte(200)?Infinity:EN(1000).pow(game.iups[1]).times(1000).pow((game.omegaChallenge==4)+1));
  get("iup3").innerHTML =
    "Multiply Incrementy multiplier by 1.2<br>Cost: " +
    beautify(EN(game.iups[2]).gte(66)?Infinity:EN(1000000000).pow(game.iups[2]).times(1000000000));
 ...
 +
  ...
  get("singFBtext").textContent = inChal(8)
    ...
    :  (game.cAutoOn.boost === 0 || game.challenge !== 0 || game.chal9 == 1 || game.chal0 == 1)
      ? "You are currently getting " + beautify(getIncrementyRate(1000).div((game.chal9) ? 1e15 : 1)) + " incrementy per second"
...
They are based on your Singularity level.`
  ...
  chalbut(-1)
  get("challenge0").style.display=(game.reachedBHO === 1?"block":"none")
  chalbut(8)
...
function iup(n, spectate = 0) {
  ...
  if (n <= 3) {
    ...
    if (game.incrementy.gte(cost) && (cost.lte(EN('1e600')) || game.incrementyverse)) {
      if (spectate == 0) {
...
function chalbut(i) {
  ...
  if (inChal(i+1) || (inChal(8) && i == 7) || (inChal(9) && i == 8) || (inChal(0) && i == -1)) {
    get("challenge" + (i + 1)).classList.add("running");
  } else if ((game.challengeCompletion[i] >= 3 && i <= 6.1)||(i==8&&game.chal9Comp==3)||(i==-1&&game.chal0Comp==3)) {
