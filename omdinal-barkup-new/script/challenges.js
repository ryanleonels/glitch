...
function exitChallenge() {
  if (game.qolSM.abu === 1 && game.leastBoost <= 5.5) {
    refundAction();
  }
  if (inAnyChal()) {
...
function completeChallenge() {
  if (game.OP >= (
    ...
    : challengeGoals[game.chal0==1?9:(game.chal9==1?8:game.challenge - 1)][game.chal0==1?game.chal0Comp:(game.chal9==1?game.chal9Comp:game.challengeCompletion[game.challenge - 1])]
  )) {
    if (game.chal8 === 1) {
      if (game.leastBoost <= 19) {
      ...
      game.chal9Comp += 1;
    } else if (game.chal0==1) {
      game.chal0Comp += 1;
    } else if (game.leastBoost <= 19) {
...
function enterChallenge(c) {
  if (!inAnyChal() && (game.challengeCompletion[c - 1] < 2.5||game.sfEver.includes(51)) && game.chal8 === 0 && game.chal9 === 0 && game.chal0 === 0 && (c%2==1||game.omegaChallenge != 2) && game.omegaChallenge != 6) {
  let conf;
...
function enterChallenge0() {
  if (!inAnyChal()&&game.omegaChallenge == 0 && game.omegaChallenge != 6 && game.reachedBHO === 1) {
  let conf;
  switch (game.bConf.chal) {
    case 1:
      conf = confirm("Are you sure you want to start a challenge? You'll " +
