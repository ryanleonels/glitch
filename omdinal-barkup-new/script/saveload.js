...
function load() {
  const localSave = localStorage[(inPublicTesting()?"omdinalBarkupPublicTestingSaveNew":"omdinalBarkupSaveNew")];
  const loadgame = (typeof localSave !== "undefined") ? JSON.parse(localSave) : null;
...
function handlePost0211Saves() {
  ...
  if (game.version === 0.341) {
    if (EN(game.iups[0]).gte(120)) game.iups[0] = EN(120)
    if (EN(game.iups[1]).gte(200)) game.iups[1] = EN(200)
    if (EN(game.iups[2]).gte(66)) game.iups[2] = EN(66)
    game.version = 1.00
  }
}
...
function save() {
  if (AF === 0) {
    localStorage.setItem((inPublicTesting()?"omdinalBarkupPublicTestingSaveNew":"omdinalBarkupSaveNew"), JSON.stringify(game))
  };
}
// hi
