...
const bupDesc = ['Each Factor\'s effect is doubled', 'Boost OP gain by 5x', 'The Ordinal Base is always 3 in Challenges', 'Dynamic Gain is boosted by Dynamic Cap',
...
function boost(f=false){
    if(data.boost.times > 37) return createAlert("Endgame!", "Congratulations, you've reached the BHO! For now, there is nothing beyond. I'd love it if you would join the Discord and let me know how you felt about the game!! :)", "Sounds good to me!")
...
function boostReq(bulkAmt = 1){
    return Math.min((3 ** (data.boost.times+bulkAmt) * 4 * 10), BHO_VALUE)
}
function getBulkBoostAmt(){
  if (!data.ord.isPsi) return 1;
  return Math.min(Math.max(Math.floor(Math.log(data.ord.ordinal/40)/Math.log(3)) - data.boost.times, 1), 38 - Math.min(data.boost.times, 37))
}
function buyBUP(i){
