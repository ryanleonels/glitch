...
let totalMult = 1
let buptotalMute = 1
const challengeGoals=[[10**32,10**223,10**270*5,10**270*5],[10**270*5,V(10)+10**270,V(17)+10**270],[10**200,10**214,1e256],[10**33,5e113,1.5e119]]
...
let EN = ExpantaNum
const BHO = V(27)
const breakBase = true // whether to break base (use normal bases) for much faster gameplay + additional layers
reset()

function reset() {
  game={
  base: (breakBase ? 10 : 100),
  ord: 0,
  over: 0,
  canInf: false,
  OP: 0,
...
function loop(ms) {
  if (game.OP > BHO-V(6)) game.OP=BHO-V(6)
  if (game.ord > BHO-V(6)) game.ord=BHO-V(6)
  ...
  incrementyMult = ((Math.log10(10+game.incrementy)**(1.05**game.iups[0]))*1.2**game.iups[2])**(game.factorBoosts>=24?(game.factor5Boost?2:1.5):1)
  buptotalMute = (game.upgrades.includes(6)?(10+game.boosters**0.9):1)*bfactorMult*incrementyMult
...
document.getElementById("buyDiagonalizeLim").innerHTML = "Permanently gain 1 maximize autobuyer<br>Cost: " + beautify(getDiagonalizeLimCost()) + " DP"
  document.getElementById("buyAdditiveProduction").innerHTML = "Each diagonalizer gives a +100% additive production boost<br>" + (game.diagonalizeAddProd == 1 ?"Bought":"Cost: 3 DP")
if (game.diagonalizeChallengeCompletion[0]==1) document.getElementById("diagonalizeChallenge1Complete").innerHTML = "Yes."
  let bfactor
  bfactorMult = 1
  for (let i=0;i<7;i++) {
    bfactor = (((1+(game.factors.length >= i+1?game.factors[i]+(game.upgrades.includes(11)?3:0):0))*(game.upgrades.includes(1) && game.factors.length >= i+1?2:1))**(challengeCurve[game.challengeCompletion[i]]*((i>=4)&&(game.factor5Boost==1)?2:1)))
...
function buyDiagonalizeSuccessor() {
  if (game.diagonalizeChallenge != 1 && game.diagonalizePoint.greaterThanOrEqualTo(getDiagonalizeSuccessorCost())) {
    ...
    game.succAuto += (game.diagonalizeChallengeCompletion[0]?(2*game.diagonalizeSucc)+1:1)
...
function buyDiagonalizeLim() {
  if (game.diagonalizeChallenge != 1 && game.diagonalizePoint.greaterThanOrEqualTo(getDiagonalizeLimCost())) {
    ...
    game.limAuto += (game.diagonalizeChallengeCompletion[0]?(2*game.diagonalizeLim)+1:1)
...
function diagonalize() {
  if (calculateDP()>=1) {
    ...
    game.canInf=false
    game.OP=0
    game.succAuto= 0 + game.diagonalizeSucc**(game.diagonalizeChallengeCompletion[0]+1)
    game.limAuto= 0 + game.diagonalizeLim**(game.diagonalizeChallengeCompletion[0]+1)
...
function factorBoost() {
  let boosted = false
  while (game.OP>=V(game.factorBoosts+3) || game.OP>=BHO-V(6)) {
    game.ord=0
    game.over=0
    game.canInf=false
    boosted = true
    ...
    game.dynamic=1
    game.incrementy=0
    if (game.factorBoosts == 15 && game.challengeCompletion[4] < 1) game.challengeCompletion[4] = 1
    if (game.factorBoosts == 16 && game.challengeCompletion[4] < 2) game.challengeCompletion[4] = 2
    if (game.factorBoosts == 17 && game.challengeCompletion[4] < 3) game.challengeCompletion[4] = 3
    if (game.factorBoosts == 18 && game.challengeCompletion[5] < 1) game.challengeCompletion[5] = 1
    if (game.factorBoosts == 19 && game.challengeCompletion[5] < 2) game.challengeCompletion[5] = 2
    if (game.factorBoosts == 20 && game.challengeCompletion[5] < 3) game.challengeCompletion[5] = 3
    if (game.factorBoosts == 21 && game.challengeCompletion[6] < 1) game.challengeCompletion[6] = 1
    if (game.factorBoosts == 22 && game.challengeCompletion[6] < 2) game.challengeCompletion[6] = 2
    if (game.factorBoosts == 23 && game.challengeCompletion[6] < 3) game.challengeCompletion[6] = 3
    if (game.factorBoosts >= 25) break
  }
  if (boosted) game.OP = 0
}
...
function bup(x,spectate=0) {
  ...
  if (!game.upgrades.includes(x)) {
    if (game.boosters>=bupUpgradeCosts[x-1] || x==12 && game.factorBoosts >= 25) {
...
function calcBase() {
  if (!breakBase) return 100
  let base=10-game.factorShifts+(game.challenge==3?5:0)+(game.challenge==4?game.factorShifts:0)
  if (base >= 8 && game.upgrades.includes(9)) base -= 4
  if (game.upgrades.includes(10)) base = 5
  return base
}
