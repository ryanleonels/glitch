...
const factorCostExp = [1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5];
...
const musicLink = [
  ...
  "https://ryanleonels.github.io/glitch2/cdn/310d7aca-4728-445f-9084-db26ceccd7b5%2FMan%20Down.mp3?v=1593650783387"
];
const musicName = [
  "OFF",
...
function loop(unadjusted, off = 0) {
  if (game.lastTick > Date.now()) {
    //document.getElementById("timehalt").innerHTML = `You have cheated by pressing Enter and Tab. As a punishment, time has been halted for ${time((game.lastTick - Date.now()) / 1000)}.`
    //document.getElementById("timehalt").style.display = "block"
  }
  else document.getElementById("timehalt").style.display = "block"
  document.getElementById("timehalt").style.transform = ""
  ...
  if (game.chal9 === 1 && game.leastBoost <= 1.5 && game.qolSM.acc === 1) {
    if (game.OP >= challengeGoals[8][Number(game.qolSM.nc8) - 1]) {
      completeChallenge();
    }
  }
  ...
  if (game.upgrades.includes(8)) {
    game.incrementy = game.incrementy.add(getIncrementyRate(ms / 2).times(1000));
  }
  changeDynamic(ms);
  ...
  buptotalMute = Math.min(
    bfactorMult *
    calcBupTotalMultWOFactor() *
    calcIncrementyMult() *
    (game.aups.includes(4) ? Math.log10(Math.log10(1e10 + game.OP)) : 1), Number.MAX_VALUE);
  ...
  if (game.upgrades.includes(8)) {
    ...
    if (game.incrementy.gte(game.bestIncrementy)) {
      game.bestIncrementy=EN(game.incrementy)
    }
  }
  ...
  if (game.cAutoLoop.shift >= 1) {
    if (!(game.advAutoShift==1&&game.factorShifts==5&&game.chal8==1)) factorShift();
...
function render() {
  ...
  if (game.boostUnlock === 1) {
    ...
    if (game.challenge === 0 && game.chal8 === 0 && game.chal9 === 0) {
  ...
  get("ordinalPointsDisplay").innerHTML =
    `You have ${beautify2(game.OP)} Ordinal Points`;
  ...
  get("factorShift").innerHTML =
    ...
    ((game.challenge == 5 || game.challenge == 7) && game.factorShifts >= 2
      ? "Infinity"
      : game.factorShifts == 7
      ? game.boostUnlock && !game.sfBought.includes(41)
        ? "Infinity"
  ...
  get("maxAllAuto").innerHTML =
    ...
    ((game.upgrades.includes(2) || game.leastBoost <= 1.5) &&
    game.autoOn.max == 1
      ? (buptotalMute >= 1e257 ? beautify(EN(buptotalMute), 3) : beautify(buptotalMute))
      : 0) +
  ...
  get("iup1").innerHTML =
    ...
    beautify(EN.pow(10, (5 * (game.iups[0] + 1))));
  get("iup2").innerHTML =
    "Double the production of incrementy<br><br>Cost: " +
    beautify(EN.pow(10, (3 * (game.iups[1] + 1))));
  get("iup3").innerHTML =
    "Multiply Incrementy multiplier by 1.2<br><br>Cost: " +
    beautify(EN.pow(10, (9 * (game.iups[2] + 1))));
  ...
  get("manifoldIncrease").textContent =
    "It is increasing by " +
    (game.upgrades.includes(13) && (game.challenge % 2 == 1 || game.chal9 == 1)
...
function iup(n, spectate = 0) {
  ...
  if (n <= 3) {
    ...
    if (game.incrementy.gte(EN.pow(iupCosts[n - 1], (game.iups[n - 1] + 1)))) {
      if (spectate == 0) {
        game.iups[n - 1] += 1;
...
function chalbut(i) {
  ...
  if (game.challenge == i + 1 || (game.chal8 == 1 && i == 7) || (game.chal9 == 1 && i == 8)) {
...
function displayOrd(
  ...
  if (ord == Infinity) {
    ...
    let ord1 = ord;
    if (ord1 >= BHO * (ordMarks[game.buchholz].length - 39)) ord1 = BHO * (ordMarks[game.buchholz].length - 39) * (1 - Number.EPSILON);
    let tempvar = Math.floor(ord1 / BHO - 1);
    let tempvar2 = 3 ** (ord1 / BHO - 1) - 3 ** tempvar;
    if (isNaN(tempvar2)) tempvar2 = 0;
    if ((ord1 / BHO) % 1 >= 0.99999999999999) {
      tempvar++;
      tempvar2 = 0;
    }
