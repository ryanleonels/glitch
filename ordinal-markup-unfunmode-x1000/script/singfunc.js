...
class SingularityFunction {
  ...
  specialReq() {
    if (this.id==41&&(getSingLevel()+game.manifolds-game.sing.m<=589||getBaseless()<=4.5)) return false
    if (this.id==51&&getSumOfChallenges()<=33.5) return false
    if (this.id==71&&getSumOfChallenges()<=35.5) return false
    if (this.id==73&&game.bestFBps<5000000000) return false
    if (this.id==81&&(getSumOfChallenges()<=64.5||game.bestIncrementy.lt("1e450"))) return false
    return true
  }
  canBuy() {
