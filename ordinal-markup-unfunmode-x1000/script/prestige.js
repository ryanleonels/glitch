...
function factorShift() {
  if (game.OP >= getFSCost(game.factorShifts) && !((game.challenge === 5 || game.challenge === 7) && game.factorShifts >= 2)) {
    if (game.factorShifts < 6.5 + (game.sfBought.includes(41) ? 1 : 0)) {
      game.ord = 0;
      game.over = 0;
      game.canInf = false;
      game.OP = 0;
...
function factorBoost(manmade = 0) {
  if (game.OP >= V(game.factorBoosts + 3, 1) && game.challenge === 0 && game.chal8 === 0 && game.chal9 === 0) {
...
function resetEverythingCollapseDoes() {
  ...
  game.challenge = 0;
  if (!game.upgrades.includes(24)) game.challengeCompletion = [0, 0, 0, 0, 0, 0, 0];
  game.incrementy = EN(0);
  game.chal8 = 0;
  game.chal9 = 0;
  if (!game.upgrades.includes(24)) game.chal8Comp = 0;
  if (!game.upgrades.includes(24)) game.chal9Comp = 0;
  game.decrementy = 0;
  game.manualClicksLeft = 1000;
  game.collapseUnlock = 1;
  game.collapseTime = 0;
