function updateMarkupHTML(){
    ...
    let ordinalCap = BHO_VALUE
    if (data.boost.times === 0) ordinalCap = GRAHAMS_VALUE
    if (data.ord.base === 2) ordinalCap = BASE2_CAP
    DOM("markupButton").innerHTML = data.ord.isPsi&&data.ord.ordinal>=ordinalCap&&data.ord.base>=2?`Base ${data.ord.base-1} is required to go further...`:
    ...
    let baseReq = "?????"
    if (data.ord.base > 3 || data.ord.base < 2) baseReq = "Infinity Ordinal Powers"
    if (data.ord.base === 3 && data.boost.times > 33) baseReq = "Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))"
    if (data.ord.base === 2 && data.boost.times > 99) baseReq = "Inaccessible Cardinal (H<sub>ψ(I)</sub>(2))"
    DOM("factorShiftButton").innerHTML = data.markup.shifts>=7?(data.boost.times>0||data.ord.base!=3)?`Perform a Factor Shift<br>Requires: ${baseReq}`:`Perform a Factor Shift<br>Requires: Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))`:
    ...
    DOM("factorBoostButton").innerHTML = `Perform ${getBulkBoostAmt() < 2 ? "a Factor Boost" : getBulkBoostAmt()+" Factor Boosts"} [+${boostersAtGivenFB(data.boost.times+getBulkBoostAmt())-boostersAtGivenFB(data.boost.times)}]<br>Requires ${displayPsiOrd(boostReq(), 3, data.boost.times > 33 ? 2 : 3)}`
    DOM("factorBoostButton").style.color = data.ord.isPsi&&(data.ord.base===2||data.boost.times<=33)&&data.ord.ordinal>=boostReq()?'#fff480':'#8080FF'
...
function markup(n=1){
    if(data.ord.base===2 && data.ord.isPsi && data.ord.ordinal >= BASE2_CAP) return data.markup.powers = 4e256
    if(data.ord.base===3 && data.ord.isPsi && data.ord.ordinal === GRAHAMS_VALUE && data.boost.times === 0) return data.markup.powers = 4e256
    if(data.ord.base===3 && data.ord.isPsi && data.ord.ordinal >= BHO_VALUE) return data.markup.powers = 4e256
    if(data.ord.ordinal<data.ord.base**2 && !data.ord.isPsi) return
    if(data.ord.isPsi){
        data.ord.ordinal+=n;
        if (data.ord.base===2 && data.ord.ordinal > BASE2_CAP) data.ord.ordinal = BASE2_CAP;
        if (data.ord.base===3 && data.ord.ordinal > GRAHAMS_VALUE && data.boost.times === 0) data.ord.ordinal = GRAHAMS_VALUE;
        if (data.ord.base===3 && data.ord.ordinal > BHO_VALUE) data.ord.ordinal = BHO_VALUE;
...
const fsReqs = [200, 1000, 1e4, 3.5e5, 1e12, 1e21, 5e100, 1e256, 1e256]
function getFSReq(){
...
function factorShift(){
    if(data.markup.shifts === 7){
        ...
        if(!data.ord.isPsi || data.ord.ordinal < GRAHAMS_VALUE || data.boost.times <= 33) return createAlert("Failure", "Insufficient Ordinal", "Dang.")
    }

    if(data.markup.shifts === 8){
        if(!data.ord.isPsi || data.ord.ordinal < 16 || data.boost.times <= 99) return createAlert("Failure", "Insufficient Ordinal", "Dang.")
    }
