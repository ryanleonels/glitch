...
function getOverflowGain(i){
    if (i == 0) return data.hierarchies.hasUpgrade[8]?getExtraBoosters()**1.1:Math.sqrt(getExtraBoosters())
...
function getOverflowEffect(i){
    ...
    switch (i) {
        case 0:
            return Math.max(1, (Math.pow(data.overflow.bp, data.hierarchies.hasUpgrade[9]?1/2.75:1/8))*getOverflowEffect(4))
        case 1:
