...
function settingsToggle(i){
    ...
    if (i == -2){
        ...
        DOM(`base2Toggle`).innerText = `Toggle Enable Base 2 on Endgame [${boolToReadable(data.base2)}]`
        if (data.ord.base === 2 && !data.base2) {
            data.ord.base = 3
            data.markup.shifts = 7
            if (data.ord.isPsi && data.ord.ordinal >= BHO_VALUE) data.ord.ordinal /= (BHO_VALUE / 8)
        }
        return save()
    }
    if (i == -3){
