const base2MinBoost = 33 // minimum amount of factor boosts required to unlock base 2
function updateMarkupHTML(){
    ...
    DOM("markupButton").innerHTML = data.ord.isPsi&&data.ord.ordinal===GRAHAMS_VALUE&&data.boost.times===0?`Base 2 is required to go further...`:
        data.ord.isPsi?`Markup and gain ${displayPsiOrd(nextPsiOrdinal(), 4)} (I)`:
    ...
    DOM("factorShiftButton").innerHTML = data.ord.base===3&&(data.boost.times<base2MinBoost||!data.base2)?data.boost.times>0?`Perform a Factor Shift<br>Requires: ?????`:`Perform a Factor Shift<br>Requires: Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))`:
    ...
    DOM("factorBoostButton").innerHTML = `Perform ${getBulkBoostAmt() < 2 ? "a Factor Boost" : getBulkBoostAmt()+" Factor Boosts"} [+${data.boost.times+1}]<br>Requires ${displayPsiOrd(boostReq(), 3, true)}`
...
function markup(n=1){
    ...
    if(data.ord.isPsi){
        if (data.ord.base === 2) {
            data.ord.psi2+=n;
            let ordList = [0,1,4,5,36,39,40,71];
            data.ord.ordinal = (BHO_VALUE * Math.floor(data.ord.psi2 / 8)) + ordList[Math.floor(data.ord.psi2 % 8)];
        } else {
            data.ord.ordinal+=n;
        }
...
function nextPsiOrdinal() {
    if (data.ord.base === 2) {
        let ord1 = data.ord.psi2+1;
        let ordList = [0,1,4,5,36,39,40,71];
        return (BHO_VALUE * Math.floor(ord1 / 8)) + ordList[Math.floor(ord1 % 8)];
    } else {
        return data.ord.ordinal+1;
    }
}
function opMult(){
    let mult = bup1Effect()
...
function factorShift(){
    if(data.markup.shifts === 7 && (data.boost.times < base2MinBoost || !data.base2)){
