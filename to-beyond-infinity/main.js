...
function loop() {
  ...
  if (game.number.OPTD == game.number.maxOPTD) {
    return
  }
 ...
 if (game.number.M.toString()=="NaN") {
   let maxOPTD = game.prestige
   if (game.prestige == game.maxPrestige) {
     maxOPTD = game.maxOPTD - 1
   }
   if (game.prestige > game.maxPrestige) {
     maxOPTD = game.maxOPTD + 1
   }
   if (game.number.OPTD >= maxOPTD) {
     document.getElementById("screen").style.display = "none"
     document.getElementById("prestige").style.display = "block"
     console.log("hi")
     if (game.number.OPTD >= 1) {
       document.getElementById("prestige").innerHTML = FindOrdinal({
          OPTD: game.number.OPTD,
          OP: 1,
          M: 1
        }).replace("(w1)", "w")
     }
     if (game.number.OPTD >= game.maxOPTD - 1) {
       document.getElementById("prestige").innerHTML = "ε0"
     }
   } else if (0==0) {
     game.number.M=1
     game.multiplier.M=1
 ...
 if (game.number.OP >= game.maxOP) {
     game.number.M=game.maxOP
     game.multiplier.M=1
     game.multBought.M=0
     game.number.OP = 0
     game.multiplier.OP = 0
     game.multBought.OP = 0
     game.multCost.M = 10 * Math.pow(0.8, game.number.OPTD + game.prestige)
     game.multCost.OP = 0
     game.number.OPTD += 1
     game.multiplier.OPTD += 1
     game.multBought.OPTD += 1
     game.multCost.OPTD += 1
 }
 if (game.number.OPTD == game.maxOPTD - 1 && game.number.OP > 0) {
     game.number.M=1
     game.multiplier.M=0
     game.multBought.M=0
     game.number.OP = 0
     ...
     game.multCost.M = 1
     game.multCost.OP = 0
     game.number.OPTD = game.maxOPTD
     game.multiplier.OPTD = game.maxOPTD
     game.multBought.OPTD = game.maxOPTD
     game.multCost.OPTD = game.maxOPTD
 }
}
...
function inf() {
  ...
  game.multBought.M=0
  game.multCost.M = 10 * Math.pow(0.8, game.prestige - 1)
  game.number.OP = 0
  game.multiplier.OP = 0
  game.multBought.OP = 0
  game.multCost.OP = 0
  game.number.OPTD = 0
  game.multiplier.OPTD = 0
  game.multBought.OPTD = 0
  game.multCost.OPTD = 0
  game.autobuyer = 0
  game.ticksInTen = 0
