...
function inc(x,y,spectate=0) {
  let incrementerCosts = incrementerCost
  if (game.fupgrades.includes(10)) {
    incrementerCosts = [5,5,5,50,100,100,250,2500,1e4,1e4,1e5,1e6] // unlock higher tiers
  }
  get(`inc${x}${y}`).classList.add("locked")
  get(`inc${x}${y}`).classList.remove("canbuy")
  if (game.factorials.gte(EN(incrementerCosts[x+4*y-9]).times(EN(incrementerCostMult[x+4*y-9]).pow(game.incrementer[x-1][y-1].bought)))) {
    get(`inc${x}${y}`).classList.remove("locked")
    get(`inc${x}${y}`).classList.add("canbuy")
    if (spectate==0) {
      game.factorials=game.factorials.minus(EN(incrementerCosts[x+4*y-9]).times(EN(incrementerCostMult[x+4*y-9]).pow(game.incrementer[x-1][y-1].bought)))
  ...
  get(`inc${x}${y}`).innerHTML=["Diagonalizer","Factor","Product","Omega Factor "][x-1] + " Incrementer " + ["I","II","III"][y-2] + "<br>" +
    ...
    beautify(EN(incrementerCosts[x+4*y-9]).times(EN(incrementerCostMult[x+4*y-9]).pow(game.incrementer[x-1][y-1].bought))) + " Factorials"
}
