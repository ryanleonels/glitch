...
function rep(mult, restOrd, base, over=0)
{
  if (EN(mult).eq(1)) {
    if (hardy(EN(base ** (base + 1)).add(restOrd), base, over) != Infinity) {
      return parseInt(beautifyEN1(hardy(EN(base ** (base + 1)).add(restOrd), base, over)).split("}}")[1])
    }
    return 3;
  }
  return beautifyEN1(hardy(EN(base ** base).times(2).add(restOrd), base, over)).split("{{").length + mult.toNumber();
}
...
function bigHardy(ord, base, over=0)
{
  ...
  //if (ord < Infinity) { // pre-psi
    ...
    if (highestPower.lt(EN(base).tetr(base)) || true) { // second condition for e.g. base 2/3 just in case (although it's normally not valid/possible)
      // 3: 1,2
      // 4: 1[2]2
      // 5: 1[1,2]2
      // 6: 1[1[2]2]2
  ...
  if (base >= 3) { // psi base 3+ - ultra-simplified, in reverse order (value represents the highest milestone below current ordinal)
...
function hugeHardy(ord, base) {
  ...
  if (ord.gte(Infinity) || ord.layer >= maxEN.layer) return "Ω"; // Absolute Infinity

  if (base === 2) { // base 2 special
    if (ord.gte(16)) return "{10,10[1[2/<sub>S<sub>Ω</sub></sub>2]2]2}"; // θ(Ω_Ω) = omega fixed point (limit of Bird's Array Notation)
