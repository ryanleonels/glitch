...
const breakBase = false // whether to break base (use normal bases) for much faster gameplay
const minBase = 2 // minimum base if breakBase = true (at least 2)
const baseReduction = false // whether to use base reduction and 90 factor shifts
const disableTetrScaling = false // disable tetrational diagonalizer cost scaling (will break late game once reaching ω^(ω^2))
reset()
...
function loop(ms) {
  ...
  if (game.fupgrades.includes(13)) game.base /= 2
  if (game.fupgrades.includes(14)) game.base /= 2
  if (isNaN(game.customBase) || !game.fupgrades.includes(15)) game.customBase=game.base
  if (isNaN(game.customBaseSelect) || !game.fupgrades.includes(15)) game.customBaseSelect=game.base
  if (game.fupgrades.includes(15)) game.base = game.customBase
  if (breakBase) {
    game.base = Math.max(Math.min(game.base, 10 - game.factorShifts), minBase)
  }
  if (baseReduction) game.base = 100 - game.factorShifts
  if (isNaN(game.autoOn.shift)) game.autoOn.shift=0
  incrementer(ms)
  if (getIncMult()>=7.77&&(game.factorials.lt(60)||game.fupgrades.includes(12))) {
    game.factorials=game.factorials.add(ms/20000)
    if (game.factorials.gte(60)&&!game.fupgrades.includes(12)) {
      game.factorials=EN(60)
    }
  }
  let factorialsCap = EN(1000).times(game.mostFactorizedOnce.gte(120)?omegaFactorMult:1)
  if (game.fupgrades.includes(12)) factorialsCap = EN(Infinity)
  if (game.mostFactorizedOnce.gte(24)&&game.factorials.lt(factorialsCap)) {
    ...
    if (game.factorials.gte(factorialsCap)) {
      game.factorials=factorialsCap
    }
  }
  ...
  let maxOrd = EN.tetr(game.base, 3) // limit ordinal to ω^(ω^ω) max as many things are broken beyond this point e.g. rapid convergence to infinity and NaNs
  if (maxOrd.lt(EN(game.base).pow(game.base*5+33))) maxOrd = EN(game.base).pow(game.base*5+33) // unless if it prevents collapse (should never happen unless in base 2-3), then raise the cap accordingly
  if (breakBase) maxOrd = EN('eee7') // raised cap for lower bases due to NaN-type errors actually occuring only beyond this point (also enables more diagonalizers and factorials at low bases)
  if (game.ord.gt(maxOrd)) game.ord = maxOrd
  if (game.dynamicUnlock==1) game.dynamic += ms/1000000+(game.iups[6]==1?99*ms/1000000:0)
  if (game.challenge==6||game.challenge==7) game.dynamic -= (10**27*ms/2)/(game.upgrades.includes(999)?10**29:1)/getManifoldEffect()
  if (game.dynamic>=10) game.dynamic = 10
  if (game.dynamic<0) game.dynamic = 0
...
function render() {
  ...
  let hardyValue = hardy(game.ord, game.base, game.over).toString()
  if (hardyValue === "Infinity") hardyValue = bigHardy(game.ord, game.base, game.over)
  document.getElementById("hardyvalue").innerHTML = "Hardy Value: " + hardyValue
  ...
  for (i in dupUpgradeCosts) {
    ...
    if (j==1&&game.diagonalUp[0].gte(65536*(1+0.5*game.pupgrades.includes(13)))&&!disableTetrScaling) {
  ...
  get("fact0").style.background=EN(game.mostFactorizedOnce).gte(1)?"green":"grey"
  get("fact1").style.background=EN(game.mostFactorizedOnce).gte(2)?"green":"grey"
  get("fact2").style.background=EN(game.mostFactorizedOnce).gte(6)?"green":"grey"
  get("fact3").style.background=EN(game.mostFactorizedOnce).gte(24)?"green":"grey"
  get("fact4").style.background=EN(game.mostFactorizedOnce).gte(120)?"green":"grey"
  get("baseSelectionButton").style.display=game.fupgrades.includes(15)?"inline":"none"
  get("currentBaseText").textContent = "Current Base: " + game.customBase;
  get("selectBaseText").textContent = "Selected Base: " + game.customBaseSelect;
  get("baseRange").value = game.customBaseSelect;
  get("chooseBase").textContent="Factorize for " + beautify(getFactorialGain()) + " Factorial(s) and Choose the Selected Base"
  get("maxDPText").textContent = "Maximum Diagonalize Points for the selected base: " + beautify(getDPcap(game.customBaseSelect)) + " Diagonalize Points"
  get("maxProdText").textContent = "Maximum Products gain for the selected base and current incrementers multipliers: " + beautify(getMaxProdGain(game.customBaseSelect)) + " Products"
  get("maxDiagText").textContent = "Maximum Diagonalizers for the selected base: at least " + beautify(getMaxDiag(game.customBaseSelect)) + " Diagonalizers"
  get("maxFactText").textContent = "Maximum Factorials gain for the selected base: at least " + beautify(getMaxFact(game.customBaseSelect)) + " Factorials"
  get("maxGainBaseText").textContent = "Maximum gain base for the selected base: at least " + beautify(getMaxGainBase(game.customBaseSelect))
  get("maxOPText").textContent = "Maximum Ordinal Points gain for the selected base: at least " + beautify(getMaxOPGain(game.customBaseSelect)) + " Ordinal Points"
}
...
function maxDup() {
  ...
  if (game.diagonalUp[0].add(x).gte(65536*(1+0.5*game.pupgrades.includes(13)))&&!disableTetrScaling) {
  ...
  if (game.diagonalUp[0].gte(1e11)) game.diagonalUp[0]=EN(1e11)
}
...
function dup(x) {
  ...
  if (x==1&&game.diagonalUp[0].gte(65536*(1+0.5*game.pupgrades.includes(13)))&&!disableTetrScaling) {
    k=EN(2).tetr(EN(5).add(game.diagonalUp[0].minus(65536*(1+0.5*game.pupgrades.includes(13))).div(1250000).div(getDiagSlowdown())))
  }
...
function getShiftCost(n) {
  if (baseReduction) {
    let factorShiftCosts1 = [
      ...
      EN('5000000000'),
      EN('20000000000'),
      EN('135181747108.12595'),
      EN('1191342213922.78'),
      EN('12727280763140.436'),
      EN('158642072774086.7'),
      EN('2252261725487611'),
      EN('3.581349445082346e16'),
      EN('6.299287353557684e17'),
      EN('1.2138260453043372e19'),
      EN('2.542604821514969e20'),
      EN('9.03521984912727e22'),
      EN('6.690076209648145e25'),
      EN('1.131383779870025e29'),
      EN('4.845194233660762e32'),
      EN('5.901672296304703e36'),
      EN('2.805902653142078e40'),
      EN('3.110855651820496e44'),
      EN('8.753220033538513e48'),
      EN('6.860955937761506e53'),
      EN('1.659675156921403e59'),
      EN('1.3868369447761608e65'),
      EN('4.5313534910073585e71'),
      EN('1e98'),
      EN('7.943282347242919e102'),
      EN('1.1091748152624055e108'),
      EN('2.8005930066362765e113'),
      EN('1.317081046294141e119'),
      EN('1e230'),
      EN('1.7782794100389228e247'),
      EN('6.219421634147599e265'),
      EN('5.349106564951274e285'),
      EN('1e400'),
      EN('1e430'),
      EN('1.77827941003869e462'),
      EN('8.293732044794982e496'),
      EN('1.540480658839498e534'),
      EN('1.7853791913427535e574'),
      EN('2.0922339930398888e617'),
      EN('4.165393384531976e663'),
      EN('2.4611054643197394e713'),
      EN('7.860768229388229e766'),
      EN('2.585978238473862e824'),
      EN('1.7521518067940427e886'),
      EN('1e1200'),
      EN('1e1320'),
      EN('1.0000000000005236e1452'),
      EN('1.584893192462939e1597'),
      EN('8.317637711041167e1756'),
      EN('4.092606597307291e1932'),
      EN('1e2050'),
      EN('1e2255'),
      EN('3.1622776601716907e2480'),
      EN('1e2660'),
      EN('1.5848931924621092e2846'),
      EN('2.716439268840311e3045'),
      EN('4.115096272813246e3258'),
      EN('1e4000'),
      EN('1e4400'),
      EN('1.000000000002094e4840'),
      EN('1.0000000000041884e5324'),
      EN('1e24000'),
      EN('1e45000'),
      EN('1e84375'),
      EN('1.333521432163324e158203'),
      EN('1e240000'),
      EN('1e324000'),
      EN('1e2880000'),
      EN('1e5299200'),
      ...
      EN('1e12700000'),
      EN('1e18034000'),
      EN('2.8313943732168383e23287432'),
      EN('1e34000000'),
      EN('1e600000000'),
      EN('1e1470000000'),
      EN('2.1906275363452514e2389612044'),
      EN('1e106000000000'),
    ];
    if (n <= 89.5) {
      return factorShiftCosts1[n]
    } else {
      return Infinity
    }
  } else {
...
function factorize() {
  ...
  if (!game.fupgrades.includes(11)) game.incrementer.forEach(a => {a.forEach(b => {b.prod=EN(1)})})
  resetEverythingCollapseDoes()
  factorMult=EN(1)
  omegaFactorMult=EN(1)
...
function factorCollapse()
{
  ...
  if (game.ord.gte(EN(game.base).pow(game.base*5+33))) // actually do the collapse
  {
    ...
    game.products = EN.isNaN(productsEarned) ? game.products.add(1) : game.products.add(productsEarned)
    resetEverythingCollapseDoes();
...
function fup(x,spectate=0) {
  ...
  if (!game.fupgrades.includes(x)) {
    if (game.factorials.gte(fupUpgradeCosts[x-1]) && (x != 15 || (game.diagonalUp[0].gte(9e6) && game.ord.gte(EN.tetr(game.base, 3))))) {
...
function beautifyEN(n,f=0) {
 let x = EN(n)
  if (x.lt(1e6)) {
    ...
    return x.layer ? x.toString() : x.toHyperE(); //"way too many";
  }
}

function beautifyEN1(n, f = 0) {
  let x = EN(n);
...
function fsubTab(t) {
  ...
  document.getElementById("fsubTab4").style.display="none"
...
function onBaseRangeChange() {
  game.customBaseSelect = parseInt(get("baseRange").value)
}

function selectBase() {
  let currentBase = game.base
  game.customBase = game.customBaseSelect
  factorize();
}

function getDPcap(base) {
  let maxOrd = EN.tetr(base, 3) // limit ordinal to ω^(ω^ω) max as many things are broken beyond this point e.g. rapid convergence to infinity and NaNs
  if (maxOrd.lt(EN(base).pow(base*5+33))) maxOrd = EN(base).pow(base*5+33) // unless if it prevents collapse (should never happen unless in base 2-3), then raise the cap accordingly
  if (breakBase) maxOrd = EN('eee7') // raised cap for lower bases due to NaN-type errors actually occuring only beyond this point (also enables more diagonalizers and factorials at low bases)
  let gain=maxOrd.div(base**2).sqrt()
  let gain0=1
  let gain1=gain
  while (gain1.gt(gain0)) {
    gain0=gain1
    gain1=gain.times(EN(4).pow(gain1.log10().add(1).floor()))
  }
  //gain=gain.times(EN(2+game.prodChalComp.includes(10)+game.prodChalComp.includes(12)).pow(game.diagonalUp[8].isNaN()?0:game.diagonalUp[8]))
  return gain1.floor()
}

function getMaxProdGain(base) {
  let maxOrd = EN.tetr(base, 3) // limit ordinal to ω^(ω^ω) max as many things are broken beyond this point e.g. rapid convergence to infinity and NaNs
  if (maxOrd.lt(EN(base).pow(base*5+33))) maxOrd = EN(base).pow(base*5+33) // unless if it prevents collapse (should never happen unless in base 2-3), then raise the cap accordingly
  if (breakBase) maxOrd = EN('eee7') // raised cap for lower bases due to NaN-type errors actually occuring only beyond this point (also enables more diagonalizers and factorials at low bases)
  let x=maxOrd.logBase(base).div(base).minus(5).div(2)
  if (x.lte(0)) return EN(0)
  return x.pow(2).add(1).pow(2).pow(1.5).times(2).times(game.incrementer[2][0].prod).floor()
}

function getMaxDiag(base) {
  let diag = getDPcap(base).add(1).logBase(2).floor()
  if (diag.lte(EN(98304))) return diag
  let prod = getMaxProdGain(base)
  let diagSlowdown = prod.add(1e10).log10().div(10).times(1.2)
  return getDPcap(base).slog(2).minus(5).times(1250000).times(diagSlowdown).add(98304).floor().add(1)
}

function getMaxFact(base) {
  let diag = getMaxDiag(base)
  if (diag.lt(1000000)) return EN(0)
  return EN.factorial(diag.div(1000000)).pow(2).times(omegaFactorMult).floor()
}

function getMaxGainBase(base) {
  let diag = getMaxDiag(base)
  let effectiveDiag=diag.times(3).times(omegaFactorMult).times(1.1).times(1.1).pow((game.omegaFactors[4]*0.03+game.omegaFactorize[4]*0.03)*(getSumOF()>=16?1.5:1)+1)
  let baseGain=EN(100)
  if (effectiveDiag.lte(8)) {
    baseGain=effectiveDiag.times(50).add(100)
  } else {
    baseGain=EN(500).times(EN(1.1).pow(effectiveDiag.minus(8)))
  }
  baseGain=baseGain.times(EN(game.incrementer[0][0].prod).pow(diag)).pow(2).pow(EN(1.000001).pow(diag))
  return baseGain.floor()
}

function getMaxOPGain(base) {
  let baseGain = getMaxGainBase(base)
  if (base == 2) return EN.pent(baseGain, 3) // 2^(2*5+33) = 2^43 > 2^^4 = 2^^^3 as the dominant term
  if (base == 3) return EN.pent(baseGain, 2) // 3^(3*5+33) = 3^48 > 3^^3 = 3^^^2 as the dominant term
  return EN.tetr(baseGain, 3) // ω^(ω^ω) > ω^(ω5+33) for bases >= 4
}
