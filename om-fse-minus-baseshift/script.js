...
function loop(ms) {
  ...
  game.base=Math.min(game.base,game.curBase)
...
function render() {
  ...
  if (game.ord.gte(EN.tetr(game.curBase, 3))) {
    document.getElementById("baseShiftButton").innerHTML = "Base Shift to increase the base to " + (game.curBase + 1)
  } else {
    document.getElementById("baseShiftButton").innerHTML = "Reach ω^ω^ω to Base Shift"
  }
...
function baseShift(debug=false) {
  if (game.ord.gte(EN.tetr(game.curBase, 3)) || debug) {
    game.ord=ENify(0)
    game.over=0
    game.canInf=false
    game.OP=ENify(0)
