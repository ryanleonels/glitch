...
function hasFactor(n){
    return data.markup.shifts >= n+1 && n < 7
}
function factorEffect(n){
    ...
    let shifts = Math.min(data.markup.shifts,7)
    return ((data.factors[n]+(1+add))*mult*bup7Effect())*(Math.max(1+(shifts-n-1)/10, 1)**[1, 1, 1, 1, 1.3, 1.9, 2.2, 2.3][shifts])
}
...
function buyMaxFactor(){
    ...
    for (let i = Math.min(data.markup.shifts-1,6); i >= 0; i--){
