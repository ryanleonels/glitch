...
let bup7Effect = () => data.boost.hasBUP[7] ? data.boost.isCharged[7] ? Math.max(1,(-data.ord.base + 6)) : Math.max(data.ord.base-2,1) : 1
let bup9Effect = () => data.boost.hasBUP[9] ? data.boost.isCharged[9] ? 20*Math.max(1,-data.ord.base+11)*getOverflowEffect(1) : 20*getOverflowEffect(1) : 1
...
function updateBoostersHTML(){
    ...
    DOM('bup3').innerText = `${data.boost.isCharged[3]?chargedBUPDesc[3]:bupDesc[3]}\n[${format(bup3Effect())}x]\n${data.boost.isCharged[3]?'':bupCosts[3]+' Boosters'}`
    ...
    for (let i = 0; i < data.autoStatus.enabled.length; i++) {
        let defaultData = getDefaultObject();
        if (i < defaultData.autoStatus.enabled.length) {
...
let boostLimit = data.trueUncapped ? Infinity : 99 // wanted to move it up to the superscaling threshold (131) due to boosted incrementy gains in base 2, but there isn't enough autobuyer speed boost for that
function boost(f=false){
    if(data.boost.times >= boostLimit) return createAlert("Endgame!", "Congratulations, you've reached the end of the current content! For now, there is nothing beyond. I'd love it if you would join the Discord and let me know how you felt about the game!! :)", "Sounds good to me!")
    if((!data.ord.isPsi || data.ord.ordinal < boostReq() || data.ord.base != 2) && !f) return createAlert("Failure", "Insufficient Ordinal", "Dang.") // base 2 only (except for the initial boost unlock)
    ...
    if (!f) {
        ...
        for(let i=1;i<=bulkBoostAmt;i++) {
            ...
            ++data.boost.times
            if(data.boost.times == 30) createAlert('Congratulations!', `You've Factor Boosted 30 times! Something new is right around the corner, but the Factor Boosts will be much harder from now on...`, 'Onwards!')
        }
    }
...
function boostReq(n = data.boost.times){
    let scaling = n < 30 ? 1 : Math.floor(2**(n-30)) // first factor boost starts at BHO / Ω_ω, the scaling is x2 below Ω_(I+1) (first 30 factor boosts are much faster) / x4 above Ω_(I+1) (slower afterwards, but endgame / FB99 has similar requirements)
    let superscaling = n < 130 ? 1 : Math.floor(2**(n-130)) // further scale it to x8 at Ω_(I+1)^I and beyond
    return 2 ** (n+1) * 2 * 2 * scaling * superscaling
}
function getBulkBoostAmt(){
    if (!data.bulkBoost || !data.ord.isPsi || data.ord.ordinal <= boostReq() || data.ord.base > 2) return 1
