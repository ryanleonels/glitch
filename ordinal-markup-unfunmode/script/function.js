...
function getIncrementyRate(x) {
  ...
  if (game.ord > BHO) ordRate = EN(BHO / 1e270).times(EN.pow(3, (game.ord / BHO - 1)));
  return EN(
    ordRate.times(x).div(1000)
      .times(EN.pow(2, game.iups[1]))
...
function calcIncrementyMult(i = game.incrementy) {
  return Math.min(ExpantaNum(i)
    .add(10)
    .log10()
    .times(ExpantaNum(1.3).pow(game.iups[2]))
    .pow(ExpantaNum(1.075).pow(game.iups[0]))
    .toNumber(), 1e120);
}
...
function calcFactorBoostTime() {
  ...
  let fbt1 = game.sfBought.includes(41) ? (117.5 / speed) : ((BHO / 1e270 / speed) * Math.min(3 ** (getSingLevel() - 1), Number.MAX_VALUE))
  return (
    Math.max(
...
function calcBase(n = game.factorShifts) {
  ...
  if (game.sfBought.includes(73) && b > 5) b = 5;
  return b;
}
