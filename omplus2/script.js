...
function loop(ms) {
  ...
  if (game.base == 2) { // activate factors gradually so that the earlier factor boosts won't pass too quickly
    let currentFB = 0;
    for (let i = 0; i < 25; i++) {
      if (game.OP >= V(i+3)) {
        currentFB++;
    ...
    for (let i = 0; i < game.factors.length; i++) {
      let curFactor = (game.factors[i] + 1) * 2;
      let curFactorExpIndex = currentFB - (i * 3) + 1;
      if (curFactorExpIndex < 0) curFactorExpIndex = 0;
      if (curFactorExpIndex > 3) curFactorExpIndex = 3;
      let curFactorExp = challengeCurve[curFactorExpIndex] * 2;
      buptotalMute *= Math.floor(curFactor ** curFactorExp);
    }
    if (currentFB >= 24) buptotalMute *= getDynamicFactorCap(); // add in dynamic factor cap to the mix for FB24-25
  }
  if ((game.succAuto < 10**265 || game.limAuto < 10**265) && !(game.ord>=game.base**game.base**game.base&&game.base<=3)) {
...
function render() {
  ...
  document.getElementById("factorMult").innerHTML = "Your factors are multiplying Tier 1 " + (game.base == 2 ? "and Tier 2 ": "")+ "Automation by " + beautify(factorMult)
  ...
  if (game.base==2) bfactorMult *= (((1+(game.factors.length >= 8?game.factors[7]+(game.upgrades.includes(11)?3:0):0))*(game.upgrades.includes(1) && game.factors.length >= 8?2:1)))
...
function displayOrd(ord,base=3,over=0,trim=0,large=0,multoff=0,colour=0) {
  ...
  } else if ((ord<base**base**base || base>3) && large==0) {
...
function factorShift() {
  if (game.OP>=factorShiftCosts[game.factorShifts] && !((game.challenge==5||game.challenge==7) && game.factorShifts >= 2)) {
    if (game.base>2) {
      ...
      if (game.base == 2) game.upgrades = [1, 2, 3]
    } else {
