...
function displayOrd(ord,over,base,trim = data.ord.trim) {
    ...
    if (base === -1 && ord >= psiValues[0]) return "-Ω"
    if (base === -2 && ord >= psiValues[1]) return "&psi;(-Ω)"
    if (base === -3 && ord >= psiValues[2]) return "&psi;(-Ω)"
    if (base < 0 && ord > Number.MAX_VALUE) return "-Ω"
    if (ord > Number.MAX_VALUE) return "Ω"
    ord = Math.floor(ord)
    over = Math.floor(over)
    if(trim <= 0) return `...`
    if(ord < Math.abs(base)) return ord+over
    const magnitude = Math.floor(Math.log(ord)/Math.log(Math.abs(base)))
    const magnitudeAmount = Math.abs(base)**magnitude
    let amount = Math.floor(ord/magnitudeAmount)*((base<0)&&(magnitude%2==1)?-1:1)
    //console.log([magnitude, amount])
    let finalOutput = (amount<0 ? "-" : "") + "&omega;"
    amount = Math.abs(amount)
    if (magnitude > 1) finalOutput += "<sup>"+displayOrd(magnitude, 0, base)+"</sup>"
    if (amount > 1) finalOutput += amount
    const firstAmount = Math.abs(amount)*magnitudeAmount
...
function calculateHardy(ord = data.ord.ordinal, over = data.ord.over, base = data.ord.base) {
    if (!base) return ord;
    if (base < 0) return NaN //calculateHardy(ord, over, -base);
...
function successor(n = 1) {
    ...
    if (data.ord.ordinal % Math.abs(data.ord.base) === Math.abs(data.ord.base) - 1) data.ord.over+=n
...
function maximize() {
    ...
    if (data.ord.over >= 1 && Math.abs(data.ord.base) === 1) {
        data.ord.ordinal = 1
        return
    }
    if (data.ord.ordinal % Math.abs(data.ord.base) === Math.abs(data.ord.base) - 1 && data.ord.over >= 1) {
        while(data.ord.over + Math.abs(data.ord.base) >= Math.abs(data.ord.base) * 2 && data.ord.ordinal % Math.abs(data.ord.base) ** 2 !== 0){
            data.ord.over -= Math.ceil((data.ord.over + Math.abs(data.ord.base)) / 2 - 0.1)
            data.ord.ordinal += Math.abs(data.ord.base)
        }

        if (data.ord.ordinal % Math.abs(data.ord.base) ** 2 !== 0) data.ord.ordinal += data.ord.over
        data.ord.over = 0
    }
}
