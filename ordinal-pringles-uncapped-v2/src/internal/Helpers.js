...
function settingsToggle(i){
    ...
    if (i == -2){ // base 2 is always enabled in this version (required for factor boosts), this one is for uncapping base 3 ordinals instead
        data.base2 = !data.base2
        DOM(`base2Toggle`).innerText = `Toggle Uncap Base 3 Ordinals After Boost [${boolToReadable(data.base2)}]`
        if (data.ord.base === 3 && data.ord.isPsi === true && data.ord.ordinal > GRAHAMS_VALUE && !data.base2) {
            data.ord.ordinal = GRAHAMS_VALUE
        }
        return save()
    }
    if (i == -3){
