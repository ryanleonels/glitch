...
function factorEffect(n){
    ...
    let shifts = data.markup.shifts
    return ((data.factors[n]+(1+add))*mult*bup7Effect())*(Math.max(1+(shifts-n-1)/10, 1)**[1, 1, 1, 1, 1.3, 1.9, 2.2, 2.3, 2.4][shifts])
}
...
function buyMaxFactor(){
    if(data.chal.active[1]) return
    if(data.ord.isPsi) return data.factors = (data.markup.shifts === 8) ? [9,8,7,7,6,6,6,6] : [9,8,7,7,6,6,6,0]
