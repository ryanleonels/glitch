function updateMarkupHTML(){
    ...
    DOM("factorShiftButton").innerHTML = (data.markup.shifts===7&&data.ord.base===3)?`Perform a Factor Shift<br>Requires: Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))`:`Perform a Factor Shift<br>Requires: ${format(getFSReq())} Ordinal Powers`
    ...
    DOM("factorBoostButton").innerHTML = `Perform ${getBulkBoostAmt() < 2 ? "a Factor Boost" : getBulkBoostAmt()+" Factor Boosts"} [+${boosterGain()}]<br>Requires ${displayPsiOrd((data.ord.base > 2 && !data.ordAboveBHO) ? Infinity : boostReq(), 3, 2)}`
