...
const chalGoals = [
    ...
    [4, 4194304, 137438953472, Infinity], //this one is in Ordinal value, [1] and [2] are post-Epsilon-Naught (slightly adjusted to give round numbers in base 2)
...
function updateChalHTML(i){
    ...
    DOM(`chal${i}`).style.backgroundColor = data.chal.active[i]?'#002480':data.chal.completions[i]===3&&i!==7?'#078228':'black'
    DOM(`chal${i}`).style.color = (!(data.chal.completions[i]===3&&i!==7)||data.chal.active[i])?'#8080FF':'black'
    ...
    DOM(`chal7`).innerHTML = `Challenge 8<br>${chalDesc[7]}<br><br>Goal: ${format(getChal8Goal(data.chal.completions[7]))} OP<br>Reward: Factor 8 and Dynamic Factor slightly boosts Tier 2 Automation<br>Completions: ${data.chal.completions[7]}/∞`
}
function chalEnter(i){
    if((data.chal.completions[i] === 3 && data.chalAfter3 != true && i !== 7) || data.chal.active.includes(true)) return
...
function chalComplete(){
    ...
    const ex = data.chal.html===1?(data.ord.isPsi&&(data.chal.completions[data.chal.html]?data.ord.base===2:true)):true
    const chalGoal = (data.chal.html === 7) ? getChal8Goal(data.chal.completions[data.chal.html]) : chalGoals[data.chal.html][data.chal.completions[data.chal.html]]
    if(currency>=chalGoal && ex){
