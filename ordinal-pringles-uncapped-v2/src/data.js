...
const VERSION = "0.0.2v2"
const VERSION_NAME = "Base 2 Edition with Factor 8"
const VERSION_DATE = "March 6th, 2024"
...
function getDefaultObject() {
    return {
        ...
        ord: {ordinal:1, over:0, base:2, trim: 5, isPsi: false, psi2: 0},
        markup: {powers:0, shifts:8},
        factors: Array(8).fill(0),
...
function save(){
    try{
        ...
        window.localStorage.setItem('ordinalPRINGLESsave2v', JSON.stringify(data))
    }
    catch (e) {
...
function load() {
    let savedata = JSON.parse(window.localStorage.getItem('ordinalPRINGLESsave2v'))
...
function fixOldSaves(){
    ...
    if (data.base2 != true && data.base2 != false) data.base2 = true
    if (data.ord.base === 2 && data.ord.ordinal > data.ord.psi2) data.ord.ordinal = data.ord.psi2
    // v0.0.4 => v0.0.5+
...
function deleteSave(){
    window.localStorage.removeItem('ordinalPRINGLESsave2v')
    location.reload()
}
