...
const boostTimesLimit = Number.MAX_VALUE
...
function boostReq(n = data.boost.times){
    ...
    return n < 33 ? (3 ** (n+1) * 4 * 10 * scaling) : ordinalCap
}
//Credit to ryanleonels
let boostLimit = () => (data.collapse.times === 0) ? 33 : 34;
function getBulkBoostAmt(){
    if (!data.sToggles[7] || !data.ord.isPsi || data.ord.ordinal.lte(boostReq())) return (data.boost.times >= 33 ? singEffects[4].effect() : 1)
