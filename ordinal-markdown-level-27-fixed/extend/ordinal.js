    ...
    while ( (((coeff[0][0][0].eq(0)&&(!this.limit))||reduceLeft.gt(0))&&!reachedZero) ) {
      switch(smallestPrincipleTerm(coeff)) {
        ...
        case 2: //Ordinal can be expressed as w^3*a+w^2*n
          ...
          break
        case 3: //Ordinal can be expressed as w^4*a+w^3*n
          cost = fgh3(coeff[0][3][0],hardyBase).minus(hardyBase)
          if (reduceLeft.gte(cost)) { //Case of term elimination
            reduceLeft = reduceLeft.minus(cost)
            hardyBase = hardyBase.add(cost)
            coeff[0][3][0] = EN(0)
          } else { //Case of non-term elimination
            amountReduceNow = affordFGH3Series(reduceLeft, hardyBase)
            cost = (!amountReduceNow.eq(0))?fgh3(amountReduceNow,hardyBase).minus(hardyBase):EN(0)
            reduceLeft = reduceLeft.minus(cost) //(new Ordinal(18,3,22,true)).toCoeff().toString()
            hardyBase = hardyBase.add(cost)
            coeff[0][3][0] = coeff[0][3][0].minus(amountReduceNow)
            if (reduceLeft.gte(1)||!this.limit) {
              coeff[0][3][0] = coeff[0][3][0].minus(1)
              coeff[0][2][0] = hardyBase
            }
          }
          break
        case -1:
...
function loga3(onum) { // Pentation log
  if(onum.array.length > 4){
    ...
    onum = EN(onum.array[2][1]).add(1);
    return EN(onum);
  } else if(onum.array.length == 2 && onum.array[1][1] >= 10){
    return EN(2);
  } else if(onum.array.length == 1 || onum.array[1][1] < 10){
    return EN(1);
  } else {
    return EN(0);
  }
...
function affordFGH3Series(amount, hardbase) {
  if (amount.lt(fgh(3,hardbase).minus(hardbase))) {
    return EN(0)
  }
  let addFGHRec = -1
  let baseFGHRec = hardbase
  while (addFGHRec < 4.5 && amount.gte(baseFGHRec.minus(hardbase)) ) {
    addFGHRec += 1
    baseFGHRec = fgh(3,baseFGHRec)
  }
  if (addFGHRec == 5) {
    //now baseFGHRec is 6 applications of fgh3 to hardbase
    let offset = loga3(fgh3(EN(6),hardbase)).sub(6)
    addFGHRec = loga3(amount).minus(offset)
    if (amount.lt(fgh3(addFGHRec, hardbase).minus(hardbase))) addFGHRec = addFGHRec.minus(1)
  }
  return EN(addFGHRec)
}
