...
let VERSION = {
	num: "0.7.1gwa",
	name: "phi with added gwa in your house",
}
...
function getBase(){
  ...
  if (inChallenge("c",11))return new Decimal(hasUpgrade("d",24)?9:10)
...
ordinal=numToOrdinal(player.points, base)+" ("+base+")"
  ...
  if (inAnyChallenge("r") || hasUpgrade("d",25)){
  ...
  if (hasUpgrade("d",12))gain=gain.mul(upgradeEffect("d",12))
    ...
    if (inAnyChallenge("r"))return gain
    else {gain = gain.pow(10)}
  }
  if (base.gte(3)){
...
function isEndgame() {
	return hasMilestone("gwa",1); // filled up your house with all the gwas
}
