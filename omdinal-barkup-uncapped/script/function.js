...
function calcCard() {
  game.maxCard = EN(
    Math.max((game.factorBoosts - 24) ** (game.factorBoosts / 25), 1) ** calcCardExponent(game.collapseTime)
  )
    .floor()
    .max(game.maxCard);
  return game.maxCard.times(1 + getFBps()).times(getFBmult()).min(1e300); // capped to prevent overflow
}

function calcCardExponent(s) {
  return 1 + (5 - Math.min(Math.log10(Math.max(1000, s)), 5)) ** 2;
}

function calcCard1(fb = game.factorBoosts, cTime = game.collapseTime) {
  let card = EN(
    Math.max((fb - 24) ** (fb / 25), 1) ** calcCardExponent(cTime)
  )
    .floor();
  return card.times(1 + getFBps()).times(getFBmult()).min(1e300); // capped to prevent overflow
}

function nextCardIn() {
  let card = calcCard();
  if (card.gte(1e300)) return Infinity;
  let x = calcCard() ** (1 / calcCardExponent(game.collapseTime));
  let fb999 = game.factorBoosts + 999;
  if (Math.max((fb999 - 24) ** (fb999 / 25), 1) <= card) return 999;
  let fb = game.factorBoosts + 1;
  while (Math.max((fb - 24) ** (fb / 25), 1) <= card && fb < game.factorBoosts + 999) fb++;
  return fb - game.factorBoosts;
}
...
function getFactorBulk() {
  if (game.OP >= V(game.factorBoosts + 3, 1)) {
    ...
    while (
      ...
      game.factorBoosts + 3 + i <= Infinity
    ) {
      i++;
    }
...
function calcOPonInfMult() {
  return (game.upgrades.includes(5) ? 50 : 1) *
        ...
        (game.sfBought.includes(42) ? Math.max(10,Math.log10(game.decrementy+1)) : 1) *
        Math.max(getSumOC(),1)
}
...
function getDecrementyRate(x) {
  return (
    ...
    (game.sfBought.includes(22) ? getManifoldEffect()**2 : 1) *
...
function getIncrementyRate(x) {
  ...
  if (game.incrementyverse==1) {
    return game.bigBrainOrd.times(
      EN(2).times(game.sfBought.includes(101)?EN(getSingLevel()).minus(game.spentENFunctions).max(3).div(2).floor():EN(1))
      .pow(game.iups[1]))
      .times(game.ivups.includes(1)?EN(1e150).pow(game.ivups.length):1).times(x/1000)//.times(1e240)//.times(1e220)
  }
  //(getSingLevel().toNumber()-2500)*0.65+2
  let ordRate = EN(game.ord / 1e270);
  if (game.ord > BHO) ordRate = EN(BHO / 1e270).times(EN(3).pow(game.ord / BHO - 1));
  return EN(
    ((ordRate.times(x/1000)))
  ) .pow(Math.max(1,Math.min(getOCComp(6),1.21)**0.5))
    ...
    .min(EN('1e597').times(x).times((game.chal9) ? 1e15 : 1));
}
...
function getBoostFromBoosters(check = 0) {
  return game.upgrades.includes(6) || check === 1
    ? Math.max(
        game.leastBoost <= 16 ? 10 + 9 * Math.max(game.boosters, 0) ** (2/3) : 0,
        10 + Math.max(game.boosters, 0)
      )
    : 1;
}
function calcFactorShiftTime(n) {
  return Math.max(
    ...
    (100 / calcOPPS(n - 1)) * (game.leastBoost <= 5.5 ? 1 : 1) +
      (calcBase(n-1)==3?3**27:OPtoOrd(getFSCost(n-1),calcBase(n-1))) /
        (calcTotalMultWOFactor() *
          ((game.upgrades.includes(1) && game.omegaChallenge != 2 ? doublex(1.5) : 1) *
            (1 + (game.upgrades.includes(11) ? doublea(3) : 0))) **
...
function calcOPPS(fs = game.factorShifts) {
  return (
    (game.upgrades.includes(5)
      ? 50 ** (inChal(1) ? (1+Math.log(125)/Math.log(50)) : 1)
      : 1) *
    game.assCard[2].mult.toNumber() *
    ((game.upgrades.includes(7) ? 120 : 0) + (game.leastBoost <= 19 ? 90 : 0)) *
  ...
  for (let i = 1; i < 9; i++) {
    ...
    if (i !== 8)
      bfact *=
        (([9, 8, 7, 4, 4, 3, 2][i - 1] +
          1 +
          (game.upgrades.includes(11) ? doublea(3) : 0)) *
          (game.upgrades.includes(1) && game.omegaChallenge != 2 ? doublex(1.5) : 1)) **
...
function getChal8Goal(x) {
  return x >= 3 && game.leastBoost >= 5.5
    ? Infinity
...
function getFBmult() {
  let x=getSingLevel() * 3
  if (game.sfBought.includes(72)) x=x**1.5
  x -= 2
  x=Math.round(x)
  if (game.omegaChallenge >= 1) return Math.max((Math.log10(Math.max(x,1))/Math.log10(Math.max((game.omegaChallenge,1)+1))),1)
  return x;
}
...
function double(x = getOCComp(2)) {
  return 2+Math.max(Math.max(x,(x-2)*2),Math.max(((x/2)-2)*(Math.pow(Math.pow(x,2)),0.5),2**(((x**0.5)/2)-2)))
}
// hi
function getSumOC() {
