...
let getPsi = ord => {
  if (game.incrementyverse==1) {
    ...
    if (game.bigBrainOrd.gte(ordThreshData["hyperoperational cutoff"])) level=156
    if (game.bigBrainOrd.gte(Infinity)) level=157
    return level
  }
...
let getPsiReq = level => {
  if (level >= 155) {
    return [ordThreshData["buchholz e(W2+1)"],ordThreshData["hyperoperational cutoff"],EN(Infinity)][level-155]
  }
  let k
  for (k in ordThresh) {
    if (level<=k) {
