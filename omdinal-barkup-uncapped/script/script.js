...
const factorCostExp = [2, 2, 2, 2, 2, 2, 2, 100];
const bupUpgradeCosts = [
  1,1,1,10,
  4,3,5,36,
  18,19,7,108,
  16,20,17,324,
  Infinity,Infinity,278,1275,
  Infinity,Infinity,1794,5050
];
...
function loop(unadjusted, off = 0) {
  ...
  if (game.OP > BHO * getSingLevel() && game.omegaChallenge == 6) game.OP = BHO * getSingLevel();
  if (game.ord > BHO * getSingLevel() && game.omegaChallenge == 6) game.ord = BHO * getSingLevel();
  if (getBaseless()>=3) {
...
function render() {
  ...
  ordColor = "no";
  const ordSub =
    game.ord <= 1e200 || fbvps >= Infinity
      ? getFBps()/getFBmult() >= Infinity
  ...
  //if (game.ord > BHO) alert(hardyValue);
  get("hardy").innerHTML =
  ...
  if (getFBps() < Infinity && game.canInf) {
  ...
  get("ordinalPointsDisplay").innerHTML =
    `You have ${fbvps>=Infinity&&game.OP>=1e270?calcOPPS()*game.msint/1000:beautify(game.OP)} Ordinal Points`;
  ...
  get("getDarkManifolds").innerHTML =
    ...
    beautifypower(game.darkManifolds >= 1e257 ? Infinity : Math.log10(game.sfBought.includes(31)?2:3) * (1 + game.darkManifolds)) +
    " Decrementy";
    ...
    (game.reachedBHO === 1
      ? "Next Cardinal in " +
        beautify(
          nextCardIn()
          /*Math.max(Math.ceil(
            ((calcCard().toNumber() / ((1 + getFBps()) * getFBmult())) + 1) **
  ...
  get("fbps").innerHTML =
    "You should have been getting a total of " +
    commafy(getFBps()) +
    " Factor Boost(s) per second if the ordinal was capped at " + displayOrd(Math.ceil(BHO * getSingLevel())) + " based on your singularity level. This amount boosts your cardinal gain multiplier";
  ...
  get("singEffect").innerHTML =
    ...
    "Raising the cardinal gain by " +
    Math.round(getFBmult())
 +
    " times" + (getSingLevel()==69?"<br><b>👀 OMG THAT'S THE NICE NUMBER!!! 👀</b>":"");
  ...
  get("sacrDM").innerHTML =
    ...
    "<br>Dark Manifolds";
  get("singFBtext").innerHTML = inChal(8)
    ...
    :  (game.cAutoOn.boost === 0 || game.challenge !== 0 || game.chal9 == 1)
      ...
      : "You currently would have been getting " + commafy(getFBps()) + " Factor Boosts per second if the ordinal was capped at " + displayOrd(Math.ceil(BHO * getSingLevel())) + " based on your singularity level.<br><br>This amount further boosts your cardinal gain, which is at " + beautify(EN(calcCard())) + " Cardinals if you collapse now";
...
function getDarkManifolds() {
  ...
  game.darkManifolds = Math.min(game.darkManifolds, 1e257);
}
...
function postBHOproj(x) {
  let goal = EN(BHO / 1e270).times(EN(3).pow(game.factorBoosts + getFactorBulk() - 24));
