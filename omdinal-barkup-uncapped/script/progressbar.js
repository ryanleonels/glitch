  ...
  if (game.OP >= V(game.factorBoosts + 3, 1) && game.bulkBoost == 0) {
    ...
    get("nextBulkTime").innerHTML =
      game.OP < 1e270
        ...
        : game.factorBoosts + getFactorBulk() >= Infinity && getFactorBulk() >= 1
        ? "You can't bulk past the BHO!"
        : "Next boost in bulk will take " +
          ((game.upgrades.includes(2) || game.leastBoost <= 5.5) && (game.autoOn.max==1) &&
          ((game.upgrades.includes(3)&&game.autoOn.inf==1) || game.leastBoost <= 1e10)
            ? time(
                game.factorBoosts < 24 && game.OP < BHO
                  ? Math.floor(
    ...
    if (game.factorBoosts >= 24 && game.OP < BHO) {
      percentage = (100 * game.OP) / BHO / 3 ** (game.factorBoosts + getFactorBulk() - 24);
    }
    if (game.factorBoosts >= 24 || game.OP >= BHO) {
      percentage = 100 * (1 / 3) ** ((BHO * (game.factorBoosts + getFactorBulk() - 23) - game.OP) / BHO);
    }
...
function challengeProject() {
  ...
  if (game.chal9==1||(game.challenge>0&&game.challengeCompletion[game.challenge-1]>=3)) {
    ...
    let autoOn = (game.upgrades.includes(2) || game.leastBoost <= 5.5) && (game.autoOn.max==1) && ((game.upgrades.includes(3)&&game.autoOn.inf==1) || game.leastBoost <= 1e10)
