...
function bigHardy(ord, base, over=0)
{
    ...
    {
        if (hardy(ord1, base, over).toString() != "Infinity") {
            return EN_format(hardy(ord1, base, over));
        }
...
function psiHardy(ord, base) {
    ...
    if (ord.gte(D(BHO_VALUE).mul(Decimal.pow(3, 156765267918869)))) return "{3,3[1[1[1/1/2/<sub>3</sub>2]2]2]2}"; // θ(θ₁(Ω^Ω)) = Ω₂^(Ω^Ω)
    ...
    if (ord.gte(BHO_VALUE * (3**493))) return "{3,3[1[1[1/2/<sub>3</sub>2]2]2]2}"; // θ(θ₁(Ω)) = θ(φ(Ω,1)) = Ω₂^Ω (highest level reachable below Number.MAX_VALUE)
