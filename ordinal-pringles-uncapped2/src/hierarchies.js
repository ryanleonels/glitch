function updateHierarchiesHTML(){
    for (let i = 0; i < data.hierarchies.ords.length; i++) {
        DOM(`h${i}Text`).innerHTML =  `${ordinalDisplay(data.hierarchies.ords[i].type, data.hierarchies.ords[i].ord, data.hierarchies.ords[i].over, data.hierarchies.ords[i].base, 3, false)} (${data.hierarchies.ords[i].base})`
    ...
    if (data.hierarchies.hasUpgrade[0]) DOM("hup0").innerHTML = `${hupData[0].text}<br>${displayHierarchyOrd(hupData[0].cost, 0, data.hierarchies.ords[0].base, 3)} FGH<br><font color='#424242'><b>Currently: ${hupData[0].effect()}x</b></font>`
    if (data.hierarchies.hasUpgrade[3]) DOM("hup3").innerHTML = `${hupData[3].text}<br>${displayHierarchyOrd(hupData[3].cost, 0, data.hierarchies.ords[1].base, 3)} SGH<br><font color='#424242'><b>Currently: ${hupData[3].effect()}x</b></font>`
}
function updateHBBuyableHTML(i){
    ...
    : `${hbData[i].text} (${formatWhole(data.hierarchies.rebuyableAmt[i])})<br>${displayHierarchyOrd(hbData[i].cost(), 0, data.hierarchies.ords[ord].base, 3)} ${cost}<br>Currently: ${format(hbData[i].effect())}x`
}
function updateHUPHTML(i){
    ...
    el.innerHTML = `${hupData[i].text}<br>${hupData[i].displayCost||displayHierarchyOrd(hupData[i].cost, 0, data.hierarchies.ords[ord].base, 3)} ${cost}<br><font color='#424242'><b>Bought!</b></font>`
}
...
function initHierarchies(){
    ...
    for (let i = 0; i < columns.length; i++) {
        ...
        for (let n = 0; n < 3; n++) {
            ...
            hb.innerHTML = n < 2 ? `${hbData[total].text} (${formatWhole(data.hierarchies.rebuyableAmt[total])})<br>${displayHierarchyOrd(hbData[total].cost(), 0, data.hierarchies.ords[i].base)} ${cost}<br>Currently: ${format(hbData[total].effect())}x`
    ...
    for (let i = 0; i < columns.length; i++) {
        ...
        for (let n = 0; n < 5; n++) {
            ...
            data.hierarchies.hasUpgrade[i+n*2] ? hup.innerHTML = `${hupData[i+n*2].text}<br>${hupData[i+n*2].displayCost||displayHierarchyOrd(hupData[i+n*2].cost, 0, data.hierarchies.ords[i].base)} ${cost}<br><font color='#424242'><b>Bought!</b></font>`
            : hup.innerHTML = `${hupData[i+n*2].text}<br>${hupData[i+n*2].displayCost||displayHierarchyOrd(hupData[i+n*2].cost, 0, data.hierarchies.ords[i].base)} ${cost}`
    ...
    for(let i=0;i<10/*data.hierarchies.hasUpgrade.length*/;i++){
...
let hierarchyData = [
    { text:"Dividing Charge Requirement by", effect: ()=> Math.max((data.hierarchies.hasUpgrade[6]?(data.hierarchies.ords[0].ord+1)**0.1:Math.log10(data.hierarchies.ords[0].ord+1))*hbData[1].effect(), 1),
        gain: ()=> hierarchyGainBases[0]()*hierarchyGainGlobalMults() },
    { text:"Multiplying Incrementy Gain by", effect: ()=> Math.max((data.hierarchies.hasUpgrade[7]?(data.hierarchies.ords[1].ord+1)**0.1:Math.log10(data.hierarchies.ords[1].ord+1))*hbData[4].effect(), 1),
...
let hupData = [
    ...
    { text:"Total Charge Boosts AutoBuyers", cost: 1e10, effect: ()=> data.hierarchies.hasUpgrade[1] ? data.incrementy.totalCharge/2 : 1 },
    { text:"Incrementy Upgrade 6 is Improved", cost: 1e20, effect: ()=> 1 },
    { text:"Incrementy Upgrade 2 is Improved", cost: 1e20, effect: ()=> 1 },
    { text:"Booster Upgrade 1x4 boosts Incrementy", cost: 1e30, displayCost: "ω<sup>ω3</sup>", effect: ()=> data.hierarchies.hasUpgrade[4] ? bup3Effect() : 1 },
    { text:"Booster Upgrade 2x4 boosts Incrementy", cost: 1e30, displayCost: "ω<sup>ω3</sup>", effect: ()=> data.hierarchies.hasUpgrade[5] ? bup7Effect() : 1 },
...
function increaseHierarchies(diff){
    for (let i = 0; i < data.hierarchies.ords.length; i++) {
        ...
        // Successor
        if (data.hierarchies.ords[i].ord % data.hierarchies.ords[i].base === data.hierarchies.ords[i].base - 1) data.hierarchies.ords[i].over+=n
        else data.hierarchies.ords[i].ord+=n
        
        //Maximize
        if (data.hierarchies.ords[i].ord % data.hierarchies.ords[i].base === data.hierarchies.ords[i].base - 1 && data.hierarchies.ords[i].over >= 1) {
            while(data.hierarchies.ords[i].over + data.hierarchies.ords[i].base >= data.hierarchies.ords[i].base * 2 && data.hierarchies.ords[i].ord % data.hierarchies.ords[i].base ** 2 !== 0){
                data.hierarchies.ords[i].over -= Math.ceil((data.hierarchies.ords[i].over + data.hierarchies.ords[i].base) / 2 - 0.1)
                data.hierarchies.ords[i].ord += data.hierarchies.ords[i].base
            }
    
            if (data.hierarchies.ords[i].ord % data.hierarchies.ords[i].base ** 2 !== 0) data.hierarchies.ords[i].ord += data.hierarchies.ords[i].over
...
function getHBBuyableCost(i){
    if(i == 2 || i==5) return 1e12*(data.hierarchies.rebuyableAmt[i]+1*10**(1+data.hierarchies.rebuyableAmt[i]))
    if(i < 3) return (data.hierarchies.rebuyableAmt[i]+1*10**(1+data.hierarchies.rebuyableAmt[i]))
    return (data.hierarchies.rebuyableAmt[i]+1*10**(1+data.hierarchies.rebuyableAmt[i]))
}
...
function buyHUP(i){
    ...
    if(data.hierarchies.ords[0].ord > cost && (i%2)==0){
        data.hierarchies.ords[0].ord -= cost
        data.hierarchies.hasUpgrade[i] = true
        updateHUPHTML(i)
    }
    if(data.hierarchies.ords[1].ord > cost && (i%2)==1){
        data.hierarchies.ords[1].ord -= cost
        data.hierarchies.hasUpgrade[i] = true
        updateHUPHTML(i)
    }
