function updateMarkupHTML(){
    ...
    let base3Cap = Infinity
    if (!data.base2) base3Cap = BHO_VALUE
    if (data.boost.times === 0) base3Cap = GRAHAMS_VALUE
    DOM("markupButton").innerHTML = data.ord.isPsi&&data.ord.ordinal>=base3Cap&&data.ord.base===3?`Base 2 is required to go further...`:
    ...
    let base2Req = "?????"
    if (data.ord.base > 3) base2Req = "Infinity Ordinal Powers"
    if (data.ord.base === 3 && data.boost.times >= 33) base2Req = displayPsiOrd(BHO_VALUE, 3)
    DOM("factorShiftButton").innerHTML = data.markup.shifts===7?(data.boost.times>0||data.ord.base!=3)?`Perform a Factor Shift<br>Requires: ${base2Req}`:`Perform a Factor Shift<br>Requires: Graham's Number (H<sub>ψ(Ω<sup>Ω</sup>ω)</sub>(3))`:
    ...
    DOM("factorBoostButton").innerHTML = `Perform ${getBulkBoostAmt() < 2 ? "a Factor Boost" : getBulkBoostAmt()+" Factor Boosts"} [+${boostersAtGivenFB(data.boost.times+getBulkBoostAmt())-boostersAtGivenFB(data.boost.times)}]<br>Requires ${displayPsiOrd((data.ord.base > 2 && !data.ordAboveBHO && data.boost.times >= 33) ? Infinity : boostReq(), 3, data.boost.times >= 33 ? 2 : 3)}`
...
function markup(n=1){
    if(data.ord.base===3 && data.ord.isPsi && data.ord.ordinal === GRAHAMS_VALUE && data.boost.times === 0) return
    if(data.ord.base===3 && data.ord.isPsi && data.ord.ordinal >= BHO_VALUE && !data.base2) return
    ...
    if(data.ord.isPsi){
        ...
        if (data.ord.base===3 && data.ord.ordinal > BHO_VALUE && !data.base2) data.ord.ordinal = BHO_VALUE;
...
function factorShift(){
    if(data.markup.shifts === 7){
        ...
        if(!data.ord.isPsi || data.ord.ordinal < BHO_VALUE || data.boost.times < 33) return createAlert("Failure", "Insufficient Ordinal", "Dang.")
    }
