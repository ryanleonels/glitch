...
let boostLimit = (data.ord.base === 2) ? (data.trueUncapped ? Infinity : 99) : 33
function boost(f=false){
    if(data.boost.times >= boostLimit) {
        if (data.ord.base === 2) return createAlert("Endgame!", "Congratulations, you've reached the end of the current content! For now, there is nothing beyond. I'd love it if you would join the Discord and let me know how you felt about the game!! :)", "Sounds good to me!")
        if (data.ord.isPsi && data.ord.ordinal >= BHO_VALUE) return createAlert("Base 2 NOW!", "Congratulations, you've reached the BHO! Base 2 is required to go further...", "To the beyond!")
    }
    if((!data.ord.isPsi || data.ord.ordinal < boostReq() || (data.boost.times >= 33 && data.ord.base != 2)) && !f) return createAlert("Failure", "Insufficient Ordinal", "Dang.") // base 2 only beyond 33 factor boosts
...
function boostReq(n = data.boost.times){
    if(n >= 77) return 2**((2*n)-43)
    if(n >= 33) return 2**(n+33)
    //if(n >= 33) return 2**(n*2)
    let scaling = n < 30 ? 1 : Math.floor(100*(n/15))
    return 3 ** (n+1) * 4 * 10 * scaling
}
function getBulkBoostAmt(){
    if (!data.bulkBoost || !data.ord.isPsi || data.ord.ordinal <= boostReq() || (data.ord.base > 2 && data.boost.times >= 33)) return 1
