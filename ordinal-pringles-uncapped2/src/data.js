...
const VERSION = "0.0.2b4"
const VERSION_NAME = "Pringles Uncapped feat. Base 2 & Bulk Boost"
const VERSION_DATE = "June 20th, 2023"
...
function getDefaultObject() {
    return {
        ...
        ord: {ordinal:1, over:0, base:10, trim: 5, isPsi: false, psi2: 0},
...
function save(){
    try{
        ...
        window.localStorage.setItem('ordinalPRINGLESsave2u', JSON.stringify(data))
    }
    catch (e) {
...
function load() {
    let savedata = JSON.parse(window.localStorage.getItem('ordinalPRINGLESsave2u'))
...
function deleteSave(){
    window.localStorage.removeItem('ordinalPRINGLESsave2u')
    location.reload()
}
